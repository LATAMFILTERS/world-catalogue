# TECHNOLOGY_CLASSIFICATION_FRAMEWORK

## Purpose

This document defines the official classification model governing all ELIMFILTERS� technologies.

The objective is to establish a consistent relationship between:

- Contamination threats
- Protection objectives
- Technologies
- Systems
- Products
- SKUs

This framework serves as the engineering foundation for the catalog, AI engine, customer intelligence platform, knowledge system, and future product development.

## Core Principle

Technologies are not products.

Technologies are engineered contamination-control platforms.

Products are physical implementations of technologies.

SKUs are commercial implementations of products.

## Official Technology Platforms

### MACROCORE�
Domain: Air Intake Protection

### SYNTAPORE�
Domain: Fuel Cleanliness Protection

### SYNTRAX�
Domain: Lubrication Protection

### NANOFORCE�
Domain: Hydraulic Protection

### HYDROCORE�
Domain: Standard Non-Turbine Fuel Water Separation

### TURBOCORE�
Domain: FH and FG Turbine Fuel Water Separation

### THERMACORE�
Domain: Cooling System Protection

### MICROKAPPA�
Domain: Cabin Environment Protection

### DRYCORE�
Domain: Compressed Air Protection

### INTEKCORE�
Domain: Airflow Management

## Governance Rule

Technology definitions remain independent of OEM references, competitor references, product branding, and commercial packaging.
