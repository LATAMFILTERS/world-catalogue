...
export function validateWeeklyEmailContract(data) {
  ...
  if (expectedPending !== pending.length) {
    throw new Error(`HERMES_EMAIL_INTEGRITY: needs_research=${expectedPending} but research_pending contains ${pending.length} detailed candidate(s).`);
  }
...
export function validateRenderedWeeklyEmail(data, message) {
  ...
  const reviewCandidates = groupCandidates(data.groups);
  const pending = asArray(data.research_pending);
  for (const item of [...reviewCandidates, ...pending]) {
    const code = String(item.entity_code || '').trim();
    if (!html.includes(code) || !text.includes(code)) {
      throw new Error(`HERMES_EMAIL_INTEGRITY: rendered email omitted detailed candidate ${code}.`);
    }
  }

  if (counts.needs_research > 0 && !visible.includes('Requiere más investigación')) {
    throw new Error('HERMES_EMAIL_INTEGRITY: pending research exists but no pending-research section was rendered.');
  }
