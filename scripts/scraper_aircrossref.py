"""
scraper_aircrossref.py — Cross-reference entre marcas de filtros de aire
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("scraper_aircrossref.log", encoding="utf-8"),
...
_EXTRACT_JS = """() => {
    const result = {};
    const links = document.querySelectorAll('ul.twocolumns li a[href*="/convert/"]');
    ...
    try:
        ...
        try:
            page.wait_for_function(
                "() => document.querySelectorAll('ul.twocolumns li a[href*=\"/convert/\"]').length > 0",
                timeout=15000
            )
        except PWTimeout:
            pass
...
if __name__ == "__main__":
    ...
    if not argv:
        print("Uso:")
        print("  python scraper_aircrossref.py air")
        print("  python scraper_aircrossref.py air-intake")
        print("  python scraper_aircrossref.py cabin")
        print("  python scraper_aircrossref.py air air-intake cabin")
        print("  python scraper_aircrossref.py --retry-zeros air")
        print("  python scraper_aircrossref.py --test P527682")
        print("  python scraper_aircrossref.py --debug P527682")
        sys.exit(0)

    if argv[0] == "--debug":
        part = argv[1] if len(argv) > 1 else "P527682"
