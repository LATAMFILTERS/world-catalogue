/**
 * apply-air-competitor-matrix.js
 *
 * Reads air_competitor_matrix.json and updates competitor_codes for
 * EA (air) filter SKUs in the DB.  Run on Render Shell.
 *
 * Run:
 *   node scripts/apply-air-competitor-matrix.js
 *   node scripts/apply-air-competitor-matrix.js --dry
 */
'use strict';
...
const MATRIX_FILE = path.join(__dirname, 'air_competitor_matrix.json');
...
async function run() {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  APPLY air competitor matrix → DB');
  console.log(`  Mode: ${DRY_RUN ? 'DRY RUN (no writes)' : 'LIVE'}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!fs.existsSync(MATRIX_FILE)) {
    console.error('air_competitor_matrix.json not found.');
    console.error('Run: node scripts/build-air-competitor-matrix.js first.');
    process.exit(1);
  }
