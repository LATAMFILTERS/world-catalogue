...
const MIGRATION = '095_GLOBAL_CANONICAL_RESOLVER_V7_SAFE';
const POLICY_VERSION = '2026-08-29-global-cert-v1.2';

async function applyGlobalCanonicalResolverV7Safe() {
  ...
  try {
    ...
    await client.query(`
      ...
      safe_ld_xref AS (
        SELECT
          r.code,
          min(r.sku)::text AS sku,
          min(r.manufacturer)::text AS manufacturer,
          900::numeric AS score,
          'RESOLVED_SINGLE'::text AS status
        FROM safe_ld_xref_raw r
        WHERE r.code<>''
        GROUP BY r.code
        HAVING count(DISTINCT r.sku)=1
      ),
    ...
    const resolverAudit = await client.query(`
      ...
      ), ambiguous_safe_xrefs AS (
        SELECT ld_catalog.norm_part(x.competitor_part_number::text) AS code
        FROM ld_catalog.ld_competitor_cross_references_safe_v x
        GROUP BY ld_catalog.norm_part(x.competitor_part_number::text)
        HAVING count(DISTINCT x.elimfilters_sku)>1
      )
      SELECT
        ...
        (SELECT count(*) FROM ambiguous_safe_xrefs)::int AS ambiguous_safe_xref_codes_excluded,
    ...
    await client.query(`
      WITH resolver_counts AS (
        ...
        SELECT
          ...
          rc.sku_count,
          rc.only_sku,
          (coalesce(rc.sku_count,0)=1 AND rc.only_sku=c.sku) AS resolver_ok
      ...
      SET
        ...
        evidence=jsonb_set(
          ...
          coalesce(to_jsonb(r.only_sku),'null'::jsonb),
          true
        ),
        policy_version='${POLICY_VERSION}',
        audited_at=now()
...
if (require.main===module) {
  applyGlobalCanonicalResolverV7Safe()
    .then(report=>console.log('[global-canonical-resolver-v7-safe]',JSON.stringify(report)))
    .catch(error=>{
      console.error('[global-canonical-resolver-v7-safe] failed',JSON.stringify(error.migrationReport || { error:error.message }));
      process.exit(1);
    });
}

module.exports={ MIGRATION, POLICY_VERSION, applyGlobalCanonicalResolverV7Safe };
