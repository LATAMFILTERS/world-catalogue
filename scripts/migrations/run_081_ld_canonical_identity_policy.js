...
/**
 * LD canonical identity policy.
 *
 * Business rules:
 * - EUROPEAN vehicle-origin families use MANN-FILTER as the canonical LD source.
 * - NON_EUROPEAN vehicle-origin families use FRAM as the canonical LD source.
 * - A canonical source reference identifies exactly one ELIMFILTERS SKU.
 * - Any equivalent code from any other manufacturer is a cross-reference only.
 * - A normalized cross-reference may resolve to only one active ELIMFILTERS SKU.
 * - Existing ambiguous legacy cross-references are quarantined instead of guessed.
 *
 * This migration does not infer vehicle origin and does not silently merge products.
 * It installs the policy, integrity guards, conflict queues and a safe resolver view.
 */

require('dotenv').config();
const { Pool } = require('pg');

const MIGRATION = '081_LD_CANONICAL_IDENTITY_POLICY';
...
async function applyLdCanonicalIdentityPolicy() {
  ...
  try {
    ...
    await client.query(`
      ...
      DROP TRIGGER IF EXISTS trg_ld_canonical_source_policy
        ON ld_catalog.ld_canonical_product_identity;
    ...
    await client.query(`
      ...
      WITH normalized AS (
        SELECT
          ...
          count(DISTINCT x.elimfilters_sku) OVER (
            PARTITION BY upper(regexp_replace(coalesce(x.competitor_brand,''), '[^A-Z0-9]', '', 'g')),
                         ld_catalog.norm_part(x.competitor_part_number)
          ) AS sku_count
        FROM ld_catalog.ld_competitor_cross_references x
      )
      SELECT id, elimfilters_sku, source_sku, competitor_brand, competitor_part_number, created_at
      FROM normalized
      WHERE normalized_reference <> '' AND sku_count = 1
    `);
    ...
    await client.query(`
      ...
      DROP TRIGGER IF EXISTS trg_ld_prevent_ambiguous_cross_reference
    ...
    await client.query(`
      ...
      WITH canonical AS (
        SELECT
          ld_catalog.norm_part(canonical_part_number) AS code,
        ...
        SELECT
          ld_catalog.norm_part(x.competitor_part_number) AS code,
...
if (require.main === module) {
  applyLdCanonicalIdentityPolicy()
    .then(report => console.log('[ld-canonical-identity-v1]', JSON.stringify(report)))
    .catch(error => {
      console.error('[ld-canonical-identity-v1] failed', JSON.stringify(error.migrationReport || { error: error.message }));
      process.exit(1);
    });
}

module.exports = { MIGRATION, applyLdCanonicalIdentityPolicy };
