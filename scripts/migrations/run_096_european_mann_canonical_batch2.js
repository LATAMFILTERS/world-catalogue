...
const MIGRATION='096_EUROPEAN_MANN_CANONICAL_BATCH2';
async function applyEuropeanMannCanonicalBatch2(){
 ...
 try{
  ...
  if(c.rows<1||c.rows!==c.normalized||c.rows!==c.mann||c.rows!==c.public) throw new Error('BATCH2_NOT_ONE_TO_ONE');
  ...
  report.checks.bad_targets=badTarget.rows[0].n; if(badTarget.rows[0].n) throw new Error('BATCH2_BAD_TARGET');
  ...
  report.checks.bad_origin=badOrigin.rows[0].n; if(badOrigin.rows[0].n) throw new Error('BATCH2_BAD_ORIGIN');
  ...
  report.checks.identity_conflicts=idConflict.rows[0].n; if(idConflict.rows[0].n) throw new Error('BATCH2_IDENTITY_COLLISION');
  const targetId=await client.query(`SELECT count(*)::int n FROM tmp_mann_b2 s JOIN ld_catalog.ld_canonical_product_identity i ON i.elimfilters_sku=s.public_sku WHERE i.status='ACTIVE' AND NOT(i.origin_group='EUROPEAN' AND ld_catalog.norm_part(i.canonical_brand)=ld_catalog.norm_part('MANN-FILTER') AND ld_catalog.norm_part(i.canonical_part_number)=ld_catalog.norm_part(s.mann_part_number))`);
  report.checks.target_identity_conflicts=targetId.rows[0].n; if(targetId.rows[0].n) throw new Error('BATCH2_TARGET_IDENTITY_COLLISION');
  ...
  report.checks.v6_matches=vr.rows[0].n; if(vr.rows[0].n!==c.rows) throw new Error('BATCH2_V6_MISMATCH');
  ...
  report.checks.v6_multi=multi.rows[0].n; if(multi.rows[0].n) throw new Error('BATCH2_V6_MULTI');
...
if(require.main===module) applyEuropeanMannCanonicalBatch2().then(r=>console.log('[european-mann-canonical-batch2]',JSON.stringify(r))).catch(e=>{console.error('[european-mann-canonical-batch2] failed',JSON.stringify(e.migrationReport||{error:e.message}));process.exit(1);});
module.exports={MIGRATION,applyEuropeanMannCanonicalBatch2};
