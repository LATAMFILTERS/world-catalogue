...
foreach($d in @('state','state\alerts','state\operations','state\operations\inbox','state\operations\processed','state\operations\approvals','state\operations\workflows','state\crm-bridge','state\crm-bridge\outbox','logs','backups','nodal-center','repos')){New-Item -ItemType Directory -Force (Join-Path $Root $d)|Out-Null}
...
if(-not(Test-Path $crm)){
  ...
  Write-Host 'Updating ELIMFILTERS CRM...'
  & git -C $crm pull --ff-only
  if($LASTEXITCODE -ne 0){throw 'CRM git pull failed; refusing to continue with mixed repository versions.'}
}
if(Test-Path (Join-Path $crm 'package.json')){Push-Location $crm;try{& npm.cmd install;if($LASTEXITCODE -ne 0){throw 'CRM npm install failed'};& npm.cmd run check;if($LASTEXITCODE -ne 0){throw 'CRM check failed'}}finally{Pop-Location}}
...
@{schema_version='1.1.0';installedAt=(Get-Date).ToUniversalTime().ToString('o');commit=(& git -C $repo rev-parse --short HEAD).Trim();crmCommit=$(if(Test-Path $crm){(& git -C $crm rev-parse --short HEAD).Trim()}else{$null});commandCenter='http://127.0.0.1:8787';watchdogMinutes=30;agentRuntimeMinutes=5;backup='daily 02:00';safeUpdate='scripts\elimserver\safe-update.ps1';crmPresent=(Test-Path $crm);crmOwnsCommercialWorkflows=$true;nodalCenter=(Join-Path $Root 'nodal-center');watchdogInitialExit=$watchExit}|ConvertTo-Json -Depth 5|Set-Content (Join-Path $Root 'state\installation.json') -Encoding UTF8
...
Write-Host 'CRM bridge: C:\ELIMSERVER\state\crm-bridge\outbox'
Write-Host 'Commercial workflows owner: C:\ELIMSERVER\repos\elimfilters-crm'
