...
const OPS = path.join(STATE, 'operations');
...
const jsonList = (dir,limit=50) => { try { return fs.readdirSync(dir).filter(f=>f.endsWith('.json')).map(f=>({data:readJson(path.join(dir,f)),mtime:fs.statSync(path.join(dir,f)).mtimeMs})).filter(x=>x.data).sort((a,b)=>b.mtime-a.mtime).slice(0,limit).map(x=>x.data); } catch { return []; } };
...
function snapshot(){
  ...
  const workflowInstances = jsonList(path.join(OPS,'workflows'),100);
  const approvals = jsonList(path.join(OPS,'approvals'),100);
  const pendingApprovals = approvals.filter(a=>a.status==='PENDING');
  const openWorkflows = workflowInstances.filter(w=>w.status!=='CLOSED');
  return {
    ...
    operations:{openWorkflows,pendingApprovals,totalWorkflows:workflowInstances.length},
...
function html(s){
  ...
  const cards=[
    ...
    ['Approvals', String(s.operations.pendingApprovals.length), 'CEO decisions pending'],
    ['Open Workflows', String(s.operations.openWorkflows.length), `Total ${s.operations.totalWorkflows}`],
...
http.createServer((req,res)=>{
  ...
  if(req.url==='/api/operations'){res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify(s.operations,null,2));}
