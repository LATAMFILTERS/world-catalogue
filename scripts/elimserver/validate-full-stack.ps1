...
foreach($p in @('state','state\operations','state\operations\inbox','state\operations\processed','state\operations\approvals','state\operations\workflows','logs','backups','nodal-center','repos\world-catalogue')){Add "path:$p" (Test-Path (Join-Path $Root $p)) (Join-Path $Root $p)}
foreach($task in @('HERMES Weekly','HERMES Recovery','ELIMSERVER Command Center','ELIMSERVER Watchdog','ELIMSERVER Backup','ELIMSERVER Agent Runtime')){try{$t=Get-ScheduledTask -TaskPath $TaskPath -TaskName $task -ErrorAction Stop;Add "task:$task" ($t.State -ne 'Disabled') "state=$($t.State)"}catch{Add "task:$task" $false $_.Exception.Message}}
...
$engine=Join-Path $repo 'scripts\elimserver\workflow-engine.mjs';Add 'workflow-engine' (Test-Path $engine) $engine
$runtime=Join-Path $repo 'scripts\elimserver\agent-runtime.mjs';Add 'agent-runtime' (Test-Path $runtime) $runtime
