/**
 * Oil/Hydraulic Filter Cross-Reference Scraper
 * Source: https://www.oilfilter-crossreference.com/convert/DONALDSON/{part}
 *
 * Para cada código Donaldson en elimfilters_catalog (Hydraulic / Lube):
 *   1. Navega a la página (Puppeteer)
 *   2. Extrae todos los pares Marca + Código de los links /convert/BRAND/CODE
 *   3. Clasifica en OEM Codes y Cross Ref Codes
 *   4. Fallback: si no hay datos, prueba alternates Donaldson del DB
 *   5. Actualiza PostgreSQL (merge con los cross-refs existentes)
 *
 * Uso:
 *   node scripts/oilfilter-crossref-scraper.js
 *   node scripts/oilfilter-crossref-scraper.js --limit 50
 *   node scripts/oilfilter-crossref-scraper.js --sku P165171
 *   node scripts/oilfilter-crossref-scraper.js --dry-run
 */
...
const OEM_BRANDS = new Set([
  ...
  "VOLVO PENTA","MERCURY","MERCRUISER","YAMAHA","SUZUKI","HONDA MARINE",
  "YANMAR MARINE","PERKINS MARINE","WESTERBEKE","NANNI","BETA MARINE",
  "PARKER","PARKER HANNIFIN","VICKERS","EATON","BOSCH REXROTH","REXROTH",
  "SAUER DANFOSS","DANFOSS","LINDE HYDRAULICS","KAWASAKI","NACHI",
  "UCHIDA","KAYABA","KYB","YUKEN","DAIKIN","TOKIMEC","MOOG","HYDAC",
  "SUNSTRAND","SUNDSTRAND","SPICER","DANA"
]);
...
const AFTERMARKET_BRANDS = new Set([
  ...
  "RYCO","TOTAL","CLARCOR","FARR","DELPHI","PALL","MP FILTRI","INTERNORMEN",
  "UFI","FILTREC","EUROFILTER","SF FILTER","ALCO","FACET","VELCON",
  "YAMASHIN","TAISEI","NIPPONDENSO","DENSO","QUALTEK","FREUDENBERG",
  "SOGEFI","API","DONALDSON","HIFI FILTER","HIFI","FLEETRITE",
  "NAPA GOLD","WESCO","SCHROEDER","HYDRAFORCE","ARGO-HYTOS","ARGO",
  "STAUFF","PARKER RACOR","RACOR","FINITE","VELCON","POROUS MEDIA",
  "ELIMFILTERS","ELIM","KLEO","KLEO FILTERS"
]);

const ELIMFILTERS_PREFIXES = [
  "EH6","EL8","EF9","ES9","EW7","ET9","EM9","EA1","EA2","EC1","ED4"
];
...
function classify(brand, code) {
  ...
  const oemKW = ["TRUCK","MOTOR","ENGINE","TRACTOR","LOADER","EXCAVATOR",
                 "CRANE","CONSTRUCTION","MACHINERY","HYDRAULIC","REXROTH"];
...
async function scrapePage(page, donaldsonCode) {
  const url = `https://www.oilfilter-crossreference.com/convert/DONALDSON/${donaldsonCode}`;
  try {
...
async function main() {
  ...
  const { rows } = await pgClient.query(`
    ...
    WHERE codigo_base IS NOT NULL
      AND filter_type IN ('Hydraulic', 'Hydraulic Filter', 'Hydraulic Housing', 'Lube')
      AND (
  ...
  await page.setExtraHTTPHeaders({
    ...
    "Referer": "https://www.oilfilter-crossreference.com/"
  });

  await page.goto("https://www.oilfilter-crossreference.com/",
  ...
  for (const row of rows) {
    ...
    if (res.error) {
      ...
      if (altCodes.length > 0) {
        ...
        const fallbackOem = [], fallbackXref = [];
        ...
        if (mergedOem.length + mergedXref.length > 0) {
          ...
          res = { oem: mergedOem, aftermarket: mergedXref,
                  total: mergedOem.length + mergedXref.length };
          if (!DRY_RUN) {
            try {
