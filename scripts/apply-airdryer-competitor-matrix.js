/**
 * apply-airdryer-competitor-matrix.js
 *
 * Reads airdryer_competitor_matrix.json and updates competitor_codes
 * for ED (air dryer) filter SKUs in the DB.  Run on Render Shell.
 *
 * Run:
 *   node scripts/apply-airdryer-competitor-matrix.js
 *   node scripts/apply-airdryer-competitor-matrix.js --dry
 */
'use strict';
...
const MATRIX_FILE = path.join(__dirname, 'airdryer_competitor_matrix.json');
...
async function run() {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  APPLY air dryer competitor matrix → DB  (ED SKUs)');
  console.log(`  Mode: ${DRY_RUN ? 'DRY RUN (no writes)' : 'LIVE'}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!fs.existsSync(MATRIX_FILE)) {
    console.error('airdryer_competitor_matrix.json not found.');
    process.exit(1);
  }
  ...
  console.log(`ED SKUs to update: ${rows.length}\n`);

  if (rows.length === 0) {
    console.log('Nothing to update — all matching ED SKUs already have competitor_codes.');
    await client.end();
    return;
  }
