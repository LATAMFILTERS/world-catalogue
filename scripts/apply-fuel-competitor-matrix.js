/**
 * apply-fuel-competitor-matrix.js
 *
 * Reads fuel_competitor_matrix.json and updates competitor_codes
 * for EF (fuel) filter SKUs in the DB.  Run on Render Shell.
 *
 * Run:
 *   node scripts/apply-fuel-competitor-matrix.js
 *   node scripts/apply-fuel-competitor-matrix.js --dry
 */
'use strict';
...
const MATRIX_FILE = path.join(__dirname, 'fuel_competitor_matrix.json');
...
async function run() {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  APPLY fuel competitor matrix → DB  (EF SKUs)');
  console.log(`  Mode: ${DRY_RUN ? 'DRY RUN (no writes)' : 'LIVE'}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!fs.existsSync(MATRIX_FILE)) {
    console.error('fuel_competitor_matrix.json not found.');
    process.exit(1);
  }
  ...
  console.log(`EF SKUs to update: ${rows.length}\n`);

  if (rows.length === 0) {
    console.log('Nothing to update — all matching EF SKUs already have competitor_codes.');
    await client.end();
    return;
  }
