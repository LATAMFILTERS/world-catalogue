/**
 * apply-hydraulic-competitor-matrix.js
 *
 * Reads hydraulic_competitor_matrix.json and updates competitor_codes
 * for EH (hydraulic) filter SKUs in the DB.  Run on Render Shell.
 *
 * Run:
 *   node scripts/apply-hydraulic-competitor-matrix.js
 *   node scripts/apply-hydraulic-competitor-matrix.js --dry
 */
'use strict';
...
const MATRIX_FILE = path.join(__dirname, 'hydraulic_competitor_matrix.json');
...
async function run() {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  APPLY hydraulic competitor matrix → DB');
  console.log(`  Mode: ${DRY_RUN ? 'DRY RUN (no writes)' : 'LIVE'}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!fs.existsSync(MATRIX_FILE)) {
    console.error('hydraulic_competitor_matrix.json not found.');
    console.error('Run: node scripts/build-hydraulic-competitor-matrix.js first.');
    process.exit(1);
  }
  ...
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log(`  APPLY COMPLETE — Updated: ${updated}`);
  console.log('═══════════════════════════════════════════════════════════\n');
