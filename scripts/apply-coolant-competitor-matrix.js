/**
 * apply-coolant-competitor-matrix.js
 *
 * Reads coolant_competitor_matrix.json and updates competitor_codes
 * for EW (coolant) filter SKUs in the DB.  Run on Render Shell.
 *
 * Run:
 *   node scripts/apply-coolant-competitor-matrix.js
 *   node scripts/apply-coolant-competitor-matrix.js --dry
 */
'use strict';
...
const MATRIX_FILE = path.join(__dirname, 'coolant_competitor_matrix.json');
...
async function run() {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  APPLY coolant competitor matrix → DB  (EW SKUs)');
  console.log(`  Mode: ${DRY_RUN ? 'DRY RUN (no writes)' : 'LIVE'}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!fs.existsSync(MATRIX_FILE)) {
    console.error('coolant_competitor_matrix.json not found.');
    process.exit(1);
  }
  ...
  console.log(`EW SKUs to update: ${rows.length}\n`);

  if (rows.length === 0) {
    console.log('Nothing to update — all matching EW SKUs already have competitor_codes.');
    await client.end();
    return;
  }
