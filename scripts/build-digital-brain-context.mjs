...
const visibilityArg = process.argv.find((value) => value.startsWith('--visibility='));
...
const routing = JSON.parse(fs.readFileSync(path.join(root, 'knowledge', 'brain', 'query-routes.json'), 'utf8'));
...
if (visibility === 'public' && selected.output_sensitivity === 'confidential') {
...
const plannedSources = [...selected.required_sources, ...(selected.optional_sources || [])]
  .map((id) => sourceMap.get(id) || { id, kind: 'derived_module', authority: 'generated_semantic_inference', sensitivity: 'internal' })
  .filter((source) => visibility !== 'public' || source.sensitivity === 'public' || source.id === 'canonical-knowledge' || source.id === 'postgresql-catalogue');
...
const pack = {
  ...
  sources: plannedSources.map(({ id, kind, authority, sensitivity, location, location_env }) => ({ id, kind, authority, sensitivity, location, location_env })),
  constraints: {
