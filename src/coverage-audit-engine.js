...
function registerCoverageAuditEngine(app, pool, limiter) {
  app.get('/api/audit/coverage-engine', limiter, async (req, res) => {
    ...
    try {
      ...
      const result = await client.query(`WITH page_skus AS (
      ...
      ORDER BY c.sku`, [make,duty,afterSku,limit]);
      ...
      for (const row of result.rows) {
        ...
        uniqueSkus.add(row.sku); if (mk) uniqueMakes.add(mk); else quality.rows_missing_make++; if (model) uniqueModels.add(mk+'|'+model); else quality.rows_missing_model++;
      ...
      const pageSkuCount=result.rows.length?Number(result.rows[0].page_sku_count||0):0;
      return res.json({success:true,engine_version:'coverage-audit-v4-direct',read_only:true,scope:{make:make||'ALL',duty,sku_limit:limit,gap_limit:gapLimit,after_sku:afterSku||null},pagination:{returned_application_rows:result.rows.length,returned_skus:pageSkuCount,has_more:pageSkuCount===limit,next_after_sku:result.rows.length?result.rows[result.rows.length-1].sku:null},summary:{...quality,unique_skus:uniqueSkus.size,unique_makes:uniqueMakes.size,unique_models:uniqueModels.size,coverage_units:units.length,status_counts:statusCounts,category_unit_counts:categoryCounts},priority_gaps:units.filter(u=>u.status!=='complete').sort((a,b)=>b.priority_score-a.priority_score||String(a.make).localeCompare(String(b.make))||String(a.model).localeCompare(String(b.model))).slice(0,gapLimit)});
    } catch (error) {
