  ...
  return {
    ...
    async enqueue(e) {
      const r = await pool.query(
        `INSERT INTO instagram_messages (event_id, type, sender_id, message_text, timestamp)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (event_id) DO NOTHING`,
        [e.event_id, e.type || 'message', e.sender_id, e.message_text, e.timestamp]
      );
      return r.rowCount === 1;
    },
    ...
    async claim(limit = 3) {
      const r = await pool.query(
        `WITH selected AS (
           SELECT event_id FROM instagram_messages
           WHERE status = 'pending' AND attempts < 3
           ORDER BY created_at
           FOR UPDATE SKIP LOCKED
           LIMIT $1
         )
         UPDATE instagram_messages j
    ...
    async status() {
      const r = await pool.query("SELECT status, COUNT(*)::int AS count FROM instagram_messages GROUP BY status");
      return Object.fromEntries(r.rows.map(x => [x.status, x.count]));
    },

    async recentDrafts(limit = 10) {
      const r = await pool.query(
        `SELECT response_text, processed_at FROM instagram_messages
