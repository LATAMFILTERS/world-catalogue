...
export function createDb(connectionString) {
  ...
  return {
    ...
    async init() {
      ...
      await pool.query(`
        CREATE TABLE IF NOT EXISTS youtube_messages (
    ...
    async enqueue(e) {
      const r = await pool.query(
        `INSERT INTO youtube_messages (event_id, type, author_channel_id, message_text, timestamp)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (event_id) DO NOTHING`,
        [e.event_id, e.type || 'comment', e.author_channel_id, e.message_text, e.timestamp]
      );
      return r.rowCount === 1;
    },
    ...
    async claim(limit = 3) {
      const r = await pool.query(
        `WITH selected AS (
           SELECT event_id FROM youtube_messages
           WHERE status = 'pending' AND attempts < 3
           ORDER BY created_at
           FOR UPDATE SKIP LOCKED
           LIMIT $1
         )
         UPDATE youtube_messages j
    ...
    async status() {
      const r = await pool.query("SELECT status, COUNT(*)::int AS count FROM youtube_messages GROUP BY status");
      return Object.fromEntries(r.rows.map(x => [x.status, x.count]));
    },

    async recentDrafts(limit = 10) {
      const r = await pool.query(
        `SELECT response_text, processed_at FROM youtube_messages
