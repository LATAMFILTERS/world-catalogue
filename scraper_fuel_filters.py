...
def run_donaldson_fuel_filters():
    with sync_playwright() as p:
        ...
        while True:
            ...
            if has_next:
                print(f"\n{'='*80}")
                print(f"📄 Página {page_num} completada. Total: {total_processed}/500")
                print(f"{'='*80}")
        ...
        print(f"\n{'='*80}")
        print(f"🏆 FUEL FILTERS SCRAPING COMPLETADO")
        print(f"{'='*80}")
        print(f"Total procesados: {total_processed}")
        print(f"Exitosos: {successful}")
        print(f"Errores: {len(results) - successful}")

        with open("fuel_filters_results.json", "w", encoding="utf-8") as f:
            json.dump(results, f, indent=4, ensure_ascii=False)

        print(f"\n📁 Guardado en: fuel_filters_results.json")
        context.close()

if __name__ == "__main__":
    run_donaldson_fuel_filters()
