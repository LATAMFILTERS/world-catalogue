/**
 * ingest-donaldson-production.ts
 * ELIMFILTERS — Donaldson Production Ingestion v1.0
 *
 * Engineering Policy v1.0 compliant.
 * Processes the real donaldson_import_ready.jsonl file.
...
async function runDonaldsonIngestion(): Promise<void> {
  const INPUT_FILE = 'C:\\Users\\VICTOR ABREU\\Documents\\world-catalogue\\scripts\\donaldson_import_ready.jsonl';
  ...
  console.log('==================================================');
  console.log('ELIMFILTERS — DONALDSON PRODUCTION INGESTION v1.0');
  ...
  for await (const line of rl) {
    ...
    if (!trimmed) continue;
    try {
      const raw = JSON.parse(trimmed) as DonaldsonRawRecord;
  ...
  // Coverage: approved / total imported
  ...
  reportLines.push(`# DONALDSON PRODUCTION INGESTION REPORT v1.0`);
  reportLines.push(`*ELIMFILTERS Engineering Policy v1.0 · All metrics VERIFIED from production data*`);
  reportLines.push(`Generated: ${new Date().toISOString()}`);
  reportLines.push(`Source File: donaldson_import_ready.jsonl`);
  reportLines.push('');
  reportLines.push('---');
  reportLines.push('');
  ...
  reportLines.push('|---|---|---|---|---|');
  reportLines.push(`| Verified OEM Catalogs | 0 | 1 | +1 | \`VERIFIED\` |`);
  reportLines.push(`| Verified Products (Approved) | 0 | ${approved.length} | +${approved.length} | \`VERIFIED\` |`);
  reportLines.push(`| Verified Products (Pending Review) | 0 | ${pendingReview.length} | +${pendingReview.length} | \`PENDING_REVIEW\` |`);
  reportLines.push(`| Verified OEM Cross References | 0 | ${totalOemXrefs} | +${totalOemXrefs} | \`VERIFIED\` |`);
  reportLines.push(`| Verified Competitor Cross References | 0 | ${totalCompetitorXrefs} | +${totalCompetitorXrefs} | \`VERIFIED\` |`);
  reportLines.push(`| Verified Equipment Applications | 0 | ${totalEquipment} | +${totalEquipment} | \`VERIFIED\` |`);
  reportLines.push(`| Verified Engines | 0 | ${allEngines.size} | +${allEngines.size} | \`VERIFIED\` |`);
  reportLines.push('');
  reportLines.push('---');
  reportLines.push('');
  reportLines.push('## 6. Evidence Summary');
  reportLines.push('');
  reportLines.push(`- **Data Source**: \`scripts/donaldson_import_ready.jsonl\``);
  reportLines.push(`- **Record Count**: ${results.length} lines parsed`);
  reportLines.push(`- **Ingestion Pipeline**: All 7 stages executed`);
  reportLines.push(`- **Policy Compliance**: Engineering Policy v1.0 enforced`);
  reportLines.push(`- **Simulated Values**: None`);
  reportLines.push('');
  reportLines.push('---');
  reportLines.push('');
  reportLines.push('## 7. Production Readiness');
  reportLines.push('');
  reportLines.push(`- **Approved records ready for production**: ${approved.length} — \`APPROVED\``);
  reportLines.push(`- **Records requiring engineering review**: ${pendingReview.length} — \`PENDING_REVIEW\``);
  reportLines.push(`- **Records excluded from production**: ${rejected.length} — \`REJECTED\``);
  reportLines.push('');
  reportLines.push('> All APPROVED records passed all 7 pipeline stages without hard errors.');
  reportLines.push('> PENDING_REVIEW records have documentation gaps (missing dimensions, no applications, or no technology mapping).');
  reportLines.push('> REJECTED records have structural errors (invalid SKU prefix or category) and require engineering correction before re-ingestion.');

  const reportContent = reportLines.join('\n');
  const reportPath = path.join(REPORT_DIR, 'donaldson_production_ingestion_report_v1.md');
  ...
  console.log('══════════════════════════════════════════════════════');
  console.log('  DONALDSON PRODUCTION INGESTION — RESULTS');
  console.log('══════════════════════════════════════════════════════');
...
runDonaldsonIngestion().catch(err => {
