...
export const PROBLEM_STUBS: ProblemStub[] = [
  // Mechanical Wear (4)
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Mobile Equipment Abrasive Wear Prevention',
        body: '⚠️ Illustrative scenario, not a documented case. Consider mobile equipment operating in a dusty environment with only OEM commodity filtration (standard air filter + engine oil filter). Under sustained dust exposure, engine teardowns can show piston ring wear of 300–400 µm against a normal baseline of roughly 50 µm, along with piston scoring and bearing surface erosion — consistent with oil cleanliness around ISO 22/20/17 against a mobile-equipment target of ISO 18/16/13. Upgrading to an ISO 5011 absolute air filter (99.5% efficiency at 5 µm vs. a typical 25 µm OEM spec), adding return-line filtration with kidney-loop offline circulation (MACROCORE + SYNTRAX combination), and increasing oil sampling frequency during high-dust periods are the standard interventions used to bring cleanliness back to target. In a scenario like this, restoring ISO 18/16/13 cleanliness would be expected to restore normal ring wear rates and extend engine overhaul intervals — the actual magnitude depends on the specific equipment, duty cycle, and starting contamination level, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    relatedStandards: ['ISO 16889', 'ISO 4406', 'ISO 19438'],
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Fleet Bearing Failure Prevention',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a mining haul-truck fleet running only commodity filtration, experiencing recurring unplanned bearing failures between planned overhaul cycles. Moving to multi-stage filtration (SYNTRAX + MACROCORE + DURATECH combination) with quarterly ISO 4406 sampling and a kidney-loop offline circulation circuit is the standard path to tightening oil cleanliness below OEM spec (into the 15/13/10 range) and controlling the abrasive/adhesive wear mechanisms that drive premature bearing failure. In a scenario like this, unplanned bearing failures would be expected to fall sharply and planned overhaul intervals could extend meaningfully once cleanliness is sustained at target — the exact interval extension, downtime reduction, and dollar savings depend on the specific fleet, duty cycle, and baseline failure rate, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Tropical Fleet Operations',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a fleet operating in a tropical climate with standard (non-desiccant) tank breathers and no routine water monitoring. During humid, rainy-season conditions, fuel tanks in this kind of situation can accumulate 300–800 ppm water, driving recurring fuel starvation and injector stiction that force unplanned downtime for injector service. Adding desiccant breathers, dual-stage HYDROCORE water-separator filtration, monthly Karl Fischer testing, and periodic fuel polishing are the standard interventions used to bring tank water down to the low tens-of-ppm range and largely eliminate water-driven shutdowns. In a scenario like this, unplanned fuel-related downtime would be expected to fall sharply once water content is controlled — the exact payback period and savings depend on fleet size, duty cycle, and baseline failure rate, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    relatedTechnologies: ['HYDROCORE™', 'SYNTAPORE™', 'DRYCORE™'],
    sections: [
      {
  ...
  // Structural Failure (4)
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Mining Wheel Loader Cavitation Damage',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a mobile equipment fleet showing progressive hydraulic noise, rough proportional control, and erratic bucket movement after several thousand hours in service. Diagnosis in a situation like this often traces to a partially restricted suction line (dropping inlet pressure below the cavitation threshold) combined with lube-only filtration (Beta 1000 at 10 µm) that leaves 15–20 µm particles in the hydraulic fluid, degrading proportional-valve cleanliness below the roughly 17/15/12 target. Correcting the suction restriction, adding a NANOFORCE offline kidney-loop (3 µm filtration) run during field operation, a high-efficiency inlet pre-filter, and a DURATECH return filter are the standard interventions used to stop cavitation noise, normalize oil TAN, and restore proportional-valve control. In a scenario like this, further pump degradation would be expected to stop once suction pressure and fluid cleanliness are corrected — the exact cost avoided depends on the specific equipment and failure stage, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Hydraulic System Contamination Control, Mining Excavator Fleet',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a fleet of large excavators operating in a highly dusty open-pit environment with recurring pump failures and heavy downtime. Fluid analysis in a situation like this can show ISO cleanliness well above the roughly 16/14/11 specification for piston pumps, along with abnormally high ferrous wear debris — often traced to a main hydraulic filter past its service life, a clogged secondary pilot-circuit filter causing bypass, and a restricted suction line. Installing a multi-stage 10 µm + 3 µm hydraulic filter system with differential-pressure gauges, adding continuous offline kidney-loop filtration (NANOFORCE, 3 µm absolute), moving to quarterly fluid sampling with ferrography, and correcting the suction restriction are the standard interventions used to bring cleanliness back to the ISO 16/14/11 target. In a scenario like this, pump failure rates would be expected to fall sharply and fluid change intervals could extend once cleanliness is sustained at target — the exact reduction, extended interval, and cost savings depend on the specific fleet, environment, and baseline failure rate, so treat any figure here as directional, not a documented result. The underlying principle holds regardless of scale: contamination control is the dominant factor in hydraulic pump reliability, and dirt ingestion is largely predictable and preventable with proper fluid management.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    relatedStandards: ['ISO 16889', 'ISO 6264', 'ISO 2941', 'SAE J1539'],
  ...
  // Chemical Degradation (2)
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Hydraulic System Overheating and Varnish Formation',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a rough-terrain lifting machine showing sluggish proportional valve response and load drift after the hydraulic system starts running well above its designed peak temperature. Investigation in a situation like this often traces to a cooler thermostatic (bypass) valve stuck partially open combined with clogged cooler fins reducing heat transfer — both of which let fluid run hot enough to accelerate oxidation, dropping RPVOT life and raising TAN, with ferrography confirming lacquer/varnish deposits. Replacing the thermostatic valve, clearing the cooler fins, flushing the system, cleaning varnish deposits from the proportional valve spool, and refilling with a fluid better suited to high-temperature service (synthetic PAO) are the standard interventions used to restore baseline operating temperature and valve response. In a scenario like this, oxidation stability and fluid life would be expected to improve substantially once temperature is brought back under control — the exact fluid-life extension and cost balance depend on the specific machine, duty cycle, and fluid chosen, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  // Turbine-Specific: FH/FG Series (4)
  {
    ...
    relatedStandards: ['ISO 5011', 'SAE J1539', 'ISO 12103'],
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: Compressor Blade Erosion, Open-Pit Mining FG Series',
        body: '⚠️ Illustrative scenario, not a documented case. Consider FG-series centrifugal compressors drawing inlet air directly from a high-dust open-pit environment with no pre-filtration. Under sustained exposure to coarse silica dust, compressor efficiency can degrade significantly within a few hundred operating hours, with blade leading-edge erosion forcing frequent shutdowns for inspection and rebuild. A staged filtration upgrade — adding a cyclone pre-separator to remove coarse (>50 µm) dust, shortening the main filter change interval, then moving to an ISO 5011 Grade 1 main filter (MACROCORE) with continuous offline kidney-loop air circulation and a desiccant inlet air dryer — is the standard path to controlling this failure mode. In a scenario like this, blade erosion would be expected to fall sharply and rebuild intervals to extend substantially as inlet air cleanliness improves at each stage — the exact reduction, extended interval, and cost savings depend on the specific site, dust loading, and compressor duty cycle, so treat any figure here as directional, not a documented result. The underlying principle holds regardless of scale: inlet filtration is far cheaper than blade replacement, and multi-stage filtration with offline circulation achieves a level of protection commodity single-stage filtration cannot match.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: FH/FG Compressor Bearing Wash-Out, Industrial Gas Compression Fleet',
        body: '⚠️ Illustrative scenario, not a documented case. Consider a fleet of process-gas compressors running continuously with no proactive oil monitoring program, experiencing bearings that seize without warning. Post-failure oil analysis in a situation like this often shows ISO cleanliness well above the roughly 16/14/11 target along with elevated TAN and meaningful viscosity loss — commonly traced to missing or clogged breather filters and leaking discharge seals letting water into the bearing cavity. Retrofitting desiccant breather filters with a sealed oil filler cap, installing ISO-certified mechanical discharge seals, and moving to a monthly fluid-sampling program (ISO 4406 count, ferrous analysis, TAN, viscosity) with bearing-sump temperature monitoring are the standard interventions used to catch degradation before failure. In a scenario like this, unexpected bearing failures would be expected to fall sharply and oil change intervals to extend once water ingress is stopped and cleanliness is sustained at target — the exact reduction and savings depend on the specific fleet, duty cycle, and baseline failure rate, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: FH/FG Compressor Seal Leakage in a Heat-Stressed Facility',
        body: '⚠️ Illustrative scenario, not a documented case. Consider compressors operating in a hot, humid climate where discharge temperature runs chronically above design (driven by high ambient inlet temperature plus compression heat rise). Sustained high discharge temperature can soften mechanical seal elastomers, letting humid discharge gas condense and leak water into the bearing cavity — visible in oil analysis as elevated water content and rising TAN in the months before a bearing seizure. Upgrading the discharge air filter to finer filtration (protecting seal faces from particle jamming), replacing standard nitrile seal elastomers with a higher-temperature-rated FKM/Viton compound, adding a discharge cooler ahead of the seal area, and deploying an offline kidney-loop with a water-absorbing cartridge on the bearing cavity oil are the standard interventions used to bring discharge temperature and bearing oil water content back within design range. In a scenario like this, bearing seizures driven by seal leakage would be expected to fall sharply once discharge temperature is controlled — the exact reduction, extended oil-change interval, and payback period depend on the specific climate, compressor duty cycle, and facility, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Illustrative Scenario: FH/FG Compressor Surge Cascade Failure in Desert Mining Operations',
        body: '⚠️ Illustrative scenario, not a documented case. Consider compressors operating in a remote, high-dust desert environment where inlet air contamination is left unaddressed. Blade erosion from coarse dust ingestion can progressively narrow the surge margin over weeks to months — starting as brief, auto-recovering surge events, then escalating in frequency and duration as blade profile distortion worsens, eventually producing a stress-concentration crack at the eroded leading edge. Left unmanaged, a sustained surge event under this kind of progressive damage can drive rotor imbalance severe enough to seize the bearing and force a full rotor, blade, and bearing rebuild. Upgrading inlet filtration (a finer-grade main filter plus a coarse pre-separator cyclone ahead of the compressor), moving to frequent borescope blade inspection, and widening the anti-surge margin in the control system are the standard interventions used to stop this cascade before it reaches rotor damage. In a scenario like this, catching blade erosion early through inlet filtration and inspection is far cheaper than a post-failure rebuild — the exact cost, downtime, and timeline depend on the specific site, dust loading, and how early the erosion is caught, so treat any figure here as directional, not a documented result.',
      },
    ],
    faqs: [
      {
