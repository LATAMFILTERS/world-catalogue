...
const TYPE_BY_KIND: Record<SchemaEntityKind, string> = {
  technology: 'DefinedTerm', system: 'DefinedTerm', family: 'ProductGroup', industry: 'DefinedTerm', standard: 'DefinedTerm', failure: 'DefinedTerm',
};

const ADDITIONAL_TYPE_BY_KIND: Record<SchemaEntityKind, string> = {
  technology: 'ELIMFILTERS Proprietary Filtration Technology', system: 'Asset Protection System', family: 'Industrial Filtration Product Family', industry: 'Industrial Operating Environment', standard: 'Engineering Standard', failure: 'Contamination Failure Mode',
};
...
function reference(node: EntityNode): SchemaReference {
  ...
  const id = node.kind === 'organization' ? ORGANIZATION_ID : canonicalEntityId(node.kind as SchemaEntityKind, slug, url);
  return { '@type': 'Thing', '@id': id, name: node.name, url };
}

export function buildOrganizationSchema() {
  return {
    '@context': 'https://schema.org' as const, '@type': 'Organization', '@id': ORGANIZATION_ID,
    name: 'ELIMFILTERS®', legalName: 'LATAMFILTERS', url: BASE_URL, logo: `${BASE_URL}/assets/logo-elimfilters.png`,
    ...
    knowsAbout: ['industrial filtration','contamination control','asset protection systems','engine air filtration','fuel cleanliness','lubrication protection','hydraulic filtration','cooling system protection'],
  };
}

export function buildWebsiteSchema() {
  return {
    '@context': 'https://schema.org' as const, '@type': 'WebSite', '@id': WEBSITE_ID,
    name: 'ELIMFILTERS World Catalogue', url: BASE_URL, publisher: { '@id': ORGANIZATION_ID },
...
export function buildCanonicalEntitySchema(kind: SchemaEntityKind, slug: string): CanonicalSchemaNode | undefined {
  ...
  const connected = [...context.systems,...context.technologies,...context.families,...context.standards,...context.industries,...context.failures];
  const about = Array.from(new Map(connected.map((node) => [node.id, reference(node)])).values());
  const id = canonicalEntityId(kind, slug, url);
  return {
    '@context': 'https://schema.org', '@type': TYPE_BY_KIND[kind], '@id': id,
    name: context.entity.name, url, description: context.definition, additionalType: ADDITIONAL_TYPE_BY_KIND[kind],
    isPartOf: { '@id': WEBSITE_ID }, provider: { '@id': ORGANIZATION_ID }, mainEntityOfPage: { '@type': 'WebPage', '@id': url },
    about: about.length ? about : undefined,
    subjectOf: context.retrievalPassages.map((passage, index) => ({ '@type': 'Thing' as const, '@id': `${id}-passage-${index + 1}`, name: passage, url })),
    knowsAbout: [context.engineeringPrinciple,context.controlStrategy,context.operationalImpact],
  };
}

export function validateCanonicalEntitySchemas(entities: ReadonlyArray<{ kind: SchemaEntityKind; slug: string }>): SchemaValidationResult {
  ...
  const invalidCanonicalUrls = present.filter((schema) => !schema.url.startsWith(BASE_URL) || schema.mainEntityOfPage['@id'] !== schema.url).map((schema) => schema['@id']);
  ...
  return { missingSchemas, duplicateIds: Array.from(new Set(duplicateIds)), invalidCanonicalUrls, invalidTechnologyEntities,
    isValid: missingSchemas.length === 0 && duplicateIds.length === 0 && invalidCanonicalUrls.length === 0 && invalidTechnologyEntities.length === 0 };
}
