...
export function buildEntitySeoGeoProfile(
  kind: SchemaEntityKind,
  slug: string,
): EntitySeoGeoProfile | undefined {
...
export function buildEntityMetadata(kind: SchemaEntityKind, slug: string): Metadata {
  ...
  return {
    ...
    alternates: {
      ...
      languages: profile.languageAlternates,
    },
    robots: {
      index: true,
      follow: true,
...
export function validateSeoGeoProfiles(
  entities: ReadonlyArray<{ kind: SchemaEntityKind; slug: string }>,
): SeoGeoValidationResult {
  const profiles = entities.map(({ kind, slug }) => ({
    key: `${kind}:${slug}`,
    profile: buildEntitySeoGeoProfile(kind, slug),
  }));

  const missingProfiles = profiles.filter(({ profile }) => !profile).map(({ key }) => key);
  const present = profiles.flatMap(({ profile }) => profile ? [profile] : []);
  const invalidCanonicalUrls = present
    .filter((profile) => !profile.canonicalUrl.startsWith(BASE_URL) || profile.entityId !== `${profile.canonicalUrl}#entity`)
