...
const CONTENT: Record<TechnologySlug, TechnologyEditorial> = {
  ...
  hydrocore: {
    problem: { title: 'Water is not just another contaminant in diesel fuel.', copy: 'It can corrode, erode, disrupt lubrication at precision fuel components and create recurring maintenance problems if separation is treated as ordinary particulate filtration. HYDROCORE™ is the ELIMFILTERS architecture for standard spin-on and cartridge fuel/water separator positions.' },
    applications: { title: 'Application scope', items: ['Spin-on fuel/water separators', 'Cartridge separator modules', 'Pre-filtration positions where the approved system requires water separation', 'Diesel equipment outside the dedicated turbine-style FH/FG architecture'] },
    ...
    selection: { title: 'How to select the correct separator', items: ['Confirm that the position is a standard fuel/water separator', 'Validate flow requirement', 'Confirm housing/module and drain or bowl configuration', 'Understand expected water load', 'Check whether a water-in-fuel sensor interface is part of the application', 'Match the downstream filtration strategy'] },
    parameters: { title: 'Engineering parameters', items: ['Water-separation performance for the validated product', 'Fuel flow', 'Contaminant capacity', 'Pressure drop', 'Water-holding/drain arrangement', 'Seal and sensor compatibility where applicable'] },
    ...
    service: { title: 'What operators should watch', items: ['Water accumulation in the bowl or drain area where provided', 'Water-in-fuel indication when the equipment supports it', 'Repeated water findings after draining', 'Corrosion or contamination during service', 'Short element life linked to dirty or wet fuel'] },
    mistakes: { title: 'Common errors in the field', items: ['Replacing a separator with an ordinary fuel filter', 'Ignoring routine draining where the system requires it', 'Assuming a new separator fixes wet bulk fuel', 'Selecting by physical dimensions without validating flow and separator function', 'Failing to investigate recurring water-in-fuel warnings'] },
    standards: { title: 'Technical reference', copy: 'Water-separation efficiency and other numeric performance claims must be tied to the specific validated product and test basis. The technology page explains the engineering function without inventing universal values.' },
    families: { title: 'Product-family connection', copy: 'HYDROCORE™ is assigned to ELIMFILTERS standard fuel/water separator families. It remains separate from plain particulate fuel filtration and from the dedicated turbine-style separator architecture.' },
    industries: { title: 'Where wet fuel creates expensive downtime', items: ['Construction', 'Mining', 'Agriculture', 'Truck fleets', 'Power generation', 'Waste and municipal', 'Bus and coach'] },
    faq: [
      ...
      { question: 'What should be checked after a water-in-fuel event?', answer: 'Follow the equipment service procedure and inspect the fuel source, separator, downstream filters and any affected fuel-system components as appropriate.' },
      { question: 'What information does ELIMFILTERS need for a separator review?', answer: 'Equipment and engine identification, current separator reference, housing/bowl arrangement, filter position, fuel source and any history of water-in-fuel events.' },
    ],
  ...
  turbocore: {
    problem: { title: 'Turbine-style fuel conditioning is an architecture, not a generic separator label.', copy: 'These systems use dedicated assemblies and staged element positions. TURBOCORE™ is applied only where the approved turbine-style FH/FG architecture requires its specific housing, element and service configuration.' },
    applications: { title: 'Where TURBOCORE™ belongs', items: ['Approved FH-style turbine separator assemblies', 'Approved FG-style turbine separator assemblies', 'Replacement elements for the designated turbine architecture', 'Applications requiring staged fuel conditioning within those assemblies'] },
    contamination: { title: 'What the assembly is managing', copy: 'The architecture is intended to condition diesel fuel through staged separation and filtration. Water and particulate loading should be considered together with flow, element position and service practice.' },
    mechanism: { title: 'Staging is the point.', copy: 'The housing geometry and element arrangement create a defined fuel path so separation and filtration occur in the intended sequence. Changing element position, rating or assembly configuration without application validation can change the protection strategy.' },
    protectedAssets: { title: 'Downstream protection', items: ['Fuel pumps', 'Injectors', 'Downstream final filtration', 'Fuel-system components exposed to contaminated or wet fuel'] },
    selection: { title: 'Selection starts with the assembly', items: ['Identify the FH/FG assembly size and configuration', 'Confirm replacement-element position', 'Confirm required flow', 'Validate the approved element rating for that position', 'Review installation orientation and service access'] },
    parameters: { title: 'Engineering parameters', items: ['Assembly flow requirement', 'Element position and validated micron rating', 'Separation duty', 'Pressure drop', 'Service access and drain arrangement', 'Housing/element interface'] },
    conditions: { title: 'Where the architecture earns its place', items: ['Fuel systems with meaningful water/particulate exposure', 'Remote equipment fueled from bulk storage', 'Marine and industrial diesel service where approved', 'Operations that benefit from accessible staged service'] },
    service: { title: 'Service discipline', items: ['Use the correct element for each stage', 'Drain collected water according to the assembly requirements', 'Inspect seals and bowl/housing condition', 'Do not mix element ratings without confirming the intended stage', 'Investigate unusually rapid loading upstream'] },
    mistakes: { title: 'Errors that defeat staged protection', items: ['Treating every turbine element as interchangeable', 'Using the wrong rating in a stage', 'Installing without adequate service clearance', 'Ignoring water drainage', 'Substituting a standard separator element into the dedicated assembly'] },
    standards: { title: 'Technical reference', copy: 'Any published flow, efficiency or element-rating claim should remain tied to the approved assembly and element. TURBOCORE™ is intentionally application-bounded rather than presented as a universal fuel-separator technology.' },
    families: { title: 'Product-family connection', copy: 'TURBOCORE™ is reserved for ELIMFILTERS turbine-style separator assemblies and their approved replacement elements. That boundary protects the taxonomy and the application decision.' },
    industries: { title: 'Typical operating contexts', items: ['Marine', 'Power generation', 'Mining support equipment', 'Construction', 'Industrial diesel systems'] },
    faq: [
      { question: 'Can I use any turbine-style element in any housing?', answer: 'No. Assembly size, element position, rating and interface must match the approved application.' },
      { question: 'Why are different element ratings used in staged systems?', answer: 'Different stages can carry different contamination-control duties. The correct sequence should follow the approved assembly configuration.' },
      { question: 'Is TURBOCORE™ the same as a standard spin-on fuel/water separator?', answer: 'No. It is restricted to its dedicated turbine-style architecture and replacement elements.' },
      { question: 'What information is needed to identify the correct element?', answer: 'Assembly reference or size, current element, stage/position, equipment application and required flow are the most useful inputs.' },
      { question: 'Why is service access important when selecting an assembly?', answer: 'Drainage, bowl inspection and element replacement must be practical in the installed location; poor access often leads to poor maintenance.' },
      { question: 'What causes unusually short element life?', answer: 'Heavy upstream contamination, wet fuel, tank condition or an incorrectly selected stage can increase loading.' },
    ],
    commercialDecision: { title: 'When to engineer the assembly rather than quote an element', copy: 'New installations, conversions and repeated contamination events should be reviewed at assembly level. ELIMFILTERS can evaluate flow, staging, serviceability and the surrounding fuel-cleanliness strategy before a turbine-style system is specified.' },
    fieldNote: { title: 'From application engineering', copy: 'With staged fuel conditioning, “it fits” is not enough. The position in the sequence is part of the specification.' },
    flow: ['applications','fieldNote','mechanism','selection','parameters','problem','contamination','conditions','service','mistakes','protectedAssets','faq','commercialDecision','families','industries','standards'],
  },
  syntrax: {
