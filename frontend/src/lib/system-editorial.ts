...
const CONTENT: Record<SystemEditorialSlug, SystemEditorial> = {
  'air-intake': {
    risk: {
      ...
      copy: 'The engineering problem is not simply whether an air element looks dirty. The real question is whether the complete intake path is controlling contaminant entry while still delivering the air mass the engine requires, since media loading, restriction, housing integrity, sealing, safety-stage condition and service practice all influence that outcome. Airborne particulate enters through the intake path or bypasses a compromised seal, and once abrasive contamination reaches turbocharger compressor surfaces, cylinders and rings, the damage mechanism is no longer a filtration issue, it is an engine-wear issue; excessive restriction creates a separate failure path by reducing available airflow and affecting performance.\n\nPrimary filtration carries the normal dust load, while secondary protection, where the equipment architecture requires it, protects the clean side during service or primary-element failure, and cabin and compressed-air positions belong to the broader airflow domain but must be selected against their own operating requirements. In the field, dust tracks on the clean side, damaged sealing surfaces, repeated early restriction, collapsed media or a housing that no longer closes correctly are system findings, not just element wear, and replacing the element without correcting the boundary can leave the asset exposed.',
    },
    fieldNote: {
    ...
    standards: {
      ...
      copy: 'Correct selection starts with exact equipment, engine and housing identification, the primary-versus-secondary element position, required airflow and allowable restriction, dust concentration and duty cycle, housing condition and seal geometry, and the service strategy and restriction-indication method already in place, since mining and quarry dust, agricultural chaff, construction environments, long highway intervals, high ambient humidity, and damaged pre-cleaning hardware all change which configuration is correct. The assets downstream of that decision include turbocharger compressor surfaces, cylinder walls and piston rings, the combustion-air path, air-cleaner housing and seals, the operator HVAC path where cabin filtration is part of the application, and pneumatic components where air drying is required.\n\nRestriction reaching the equipment service threshold, dust evidence on the clean side, damaged seals, and recurring premature plugging are the signals worth acting on, not appearance alone; the most common mistakes are selecting by dimensions only, ignoring housing or seal damage, removing a safety element because it looks clean, and extending service past the restriction limit. Engine air-cleaner performance is commonly evaluated using ISO 5011 methods and claims should be tied to validated data, not applied universally; airflow protection is operationally critical across mining, construction, agriculture, truck fleets, power generation, waste and municipal fleets, and bus and coach operations, and when a fleet is experiencing repeated dust ingress or intake-related wear, ELIMFILTERS can review the complete airflow boundary to select the correct protection architecture, not simply quote another replacement element.',
    },
    industries: {
  ...
  'fuel-cleanliness': {
    risk: {
      ...
      copy: 'Modern fuel systems have little tolerance for abrasive particles, free water or unstable fuel quality. The correct protection strategy is staged around the failure path, controlling particulate loading, removing water where required, preserving flow and keeping contamination away from high-pressure pumps and injectors.\n\nContamination can enter during fuel transfer, storage, tank breathing or service. Water promotes corrosion and microbial growth inside the tank and lines, while hard particles accelerate abrasive wear in precision metering components, and as system pressure increases downstream toward the injectors, the consequences of the same contamination event become more severe.\n\nA plugged fine filter and recurring water are two different failure signatures, not the same problem with two symptoms. Repeated plugging usually points to upstream particulate ingress, while recurring water after draining points to storage, condensation, tank ingress or poor drainage practice further back in the fuel supply chain. Replacing the last element in the chain corrects neither cause.\n\nParticulate filtration and water separation solve different problems and must not be treated as interchangeable. Standard fuel/water separation, turbine-specific separation architectures, and primary/secondary particulate filtration each occupy a defined position in the system, and flow, water-holding capacity, micron requirement, drainage and service access all have to work together for that staged strategy to hold.',
    },
    fieldNote: {
    ...
    standards: {
      title: 'Selecting and servicing the correct configuration',
      copy: 'Selecting the right configuration starts with confirming the engine, fuel-system architecture and existing filter positions, then identifying whether the application needs particulate control, water separation, or both. Required flow and pressure conditions, current fuel quality and storage practices, and service access, drainage and water-monitoring needs all shape the final specification, not just the reference already installed.\n\nFuel flow, particle-efficiency requirement, water-separation requirement, water-holding capacity, pressure drop, fuel temperature and viscosity, and service-interval and drain strategy are the parameters that actually drive the decision. Bulk fuel storage, high humidity and condensation exposure, remote fuel-quality variation, high-hour stationary engines, mining and off-road refueling, and marine or power-generation environments with extended storage each raise the bar differently.\n\nThe components exposed to that decision are the high-pressure fuel pump, injectors and precision control valves, fuel rails and metering components, transfer pumps, and downstream particulate filters, with combustion stability affected indirectly through fuel quality. Recurring water in the separator, premature plugging, hard starting or loss of power under load, visible contamination in drained fuel, and shortening service intervals without a duty-cycle change are the signals that justify investigation.\n\nA separator and a fuel filter are not the same component and can coexist in the same protection architecture without being interchangeable, and the finest available micron rating is not automatically the right choice once efficiency, flow, pressure drop and capacity are weighed together. The most common mistakes are assuming a finer element automatically improves the system, ignoring water because the engine still runs, using a separator where only particulate filtration is required or vice versa, and selecting by thread or external dimensions without validating flow and architecture.\n\nFuel-filter and water-separation performance claims should remain tied to the applicable validated test basis for the specific product or assembly, since particle control and water separation are separate engineering questions. Fuel-cleanliness protection is operationally critical across truck fleets, mining, construction, agriculture, power generation, marine, oil and gas, and waste and municipal fleets. Recurring injector failures, water events, premature plugging or unstable fuel quality justify a full review of the contamination path and filtration stages, not just a cross-reference exercise.',
    },
    industries: {
  ...
  lubrication: {
    risk: {
      ...
      copy: 'Engine oil carries soot, wear debris and oxidation products while temperature and viscosity move continuously through the duty cycle. A lubrication system has to control contamination without compromising oil delivery, bypass behavior or pressure stability.\n\nParticles generated at bearings, rings, gears and other loaded interfaces are carried through the same fluid that protects the engine. If contamination is not controlled, abrasive and fatigue wear can accelerate, and if restriction becomes excessive, flow or bypass behavior can create a different risk entirely.\n\nMedia efficiency, contaminant capacity, structural integrity, pressure drop and valve behavior must remain compatible with oil viscosity and operating temperature. The goal is not maximum filtration in isolation, it is stable protection across the full service interval.\n\nA filter cannot correct a lubrication problem created elsewhere. Fuel dilution, coolant contamination, excessive soot loading, wrong viscosity or abnormal wear can overwhelm the filtration strategy, so element condition should always be interpreted together with oil condition and engine behavior, not read in isolation.',
    },
    fieldNote: {
    ...
    standards: {
      title: 'Selecting and servicing the correct configuration',
      copy: 'Selection starts with confirming the engine and exact filter position, then validating thread, sealing and bypass requirements against the oil grade and expected viscosity range. Soot and contamination load from the duty cycle, service history and oil analysis where available should all inform the decision, and intervals should never be extended without supporting evidence.\n\nOil flow, pressure drop, media efficiency, contaminant capacity, bypass-valve requirement, operating viscosity and temperature, and structural integrity under pressure are the parameters that actually drive selection. High idle time, heavy load and high thermal duty, cold starts, soot-intensive operation, extended service intervals, and fuel dilution or coolant contamination events all raise lubrication stress differently.\n\nThe assets at risk are the main and connecting-rod bearings, turbocharger bearings, camshaft and valve-train interfaces, piston and ring lubrication zones, the oil pump and galleries, and other pressure-lubricated engine surfaces. Abnormal oil pressure, repeatedly shortened filter life, unusual debris in the removed element, oil analysis showing increasing wear metals or contamination, and evidence of dilution or coolant ingress are signals that deserve more than a routine filter change.\n\nA better oil filter does not by itself justify a longer oil-change interval; oil condition, engine requirements, duty cycle and validated maintenance guidance still govern that decision. Bypass behavior matters because the system must preserve oil delivery under changing viscosity and restriction, and abnormal debris in a used filter should be read as evidence of upstream wear or contamination, not simply discarded. The most common mistakes are extending drain intervals based only on filter claims, ignoring bypass-valve compatibility, selecting by thread and gasket dimensions only, and assuming a filter can compensate for incorrect oil or mechanical wear.\n\nLubrication-filter performance should be evaluated using the validated test basis appropriate to the specific product and application, since published efficiency or capacity values must not be generalized beyond the evidence available for the element. Lubrication reliability carries high operational value across truck fleets, mining, construction, agriculture, power generation, bus and coach, and waste and municipal fleets. When oil pressure, abnormal wear, shortened service life or contamination trends are creating uncertainty, ELIMFILTERS can review the filtration application alongside the operating condition, so the commercial decision follows the engineering evidence instead of preceding it.',
    },
    industries: {
