...
const CONTENT: Record<SystemEditorialSlug, SystemEditorial> = {
  'air-intake': {
    risk: {
      ...
      copy: 'The engineering problem is not simply whether an air element looks dirty. The real question is whether the complete intake path is controlling contaminant entry while still delivering the air mass the engine requires, since media loading, restriction, housing integrity, sealing, safety-stage condition and service practice all influence that outcome. Airborne particulate enters through the intake path or bypasses a compromised seal, and once abrasive contamination reaches turbocharger compressor surfaces, cylinders and rings, the damage mechanism is no longer a filtration issue, it is an engine-wear issue. Excessive restriction creates a separate failure path by reducing available airflow and affecting performance. Primary filtration carries the normal dust load, while secondary protection, where the equipment architecture requires it, protects the clean side during service or primary-element failure. Housings, inlet geometry, sealing surfaces and restriction monitoring determine whether the media can perform as designed, and cabin and compressed-air positions belong to the broader airflow domain but must be selected against their own operating requirements. In the field, dust tracks on the clean side, damaged sealing surfaces, repeated early restriction, collapsed media or a housing that no longer closes correctly are system findings, not just element wear. Replacing the element without correcting the boundary can leave the asset exposed.',
    },
    fieldNote: {
