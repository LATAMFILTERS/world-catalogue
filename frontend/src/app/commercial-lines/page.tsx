...
export default function CommercialLinesPage() {
  const schema = {
    ...
    hasPart: SOLUTIONS.map((solution) => ({ '@type': 'WebPage', name: solution.name, url: `${BASE_URL}${solution.href}`, description: solution.description })),
  };
  ...
  return (
    <main style={main}>
      ...
      <section style={hero}><div style={heroInner}><p style={eyebrow}>// SPECIALIZED ASSET PROTECTION SOLUTIONS</p><h1 style={heroTitle}>Specialized operating environments require governed application logic.</h1><p style={heroLead}>MARINECLEAN™ and DURACTECH™ extend the five core ELIMFILTERS protection systems without creating additional core technologies or systems.</p></div></section>
      <section style={section}><div style={grid}>{SOLUTIONS.map((solution,index)=><Link key={solution.slug} href={solution.href} style={card}><div style={cardHeader}><span style={number}>{String(index+1).padStart(2,'0')}</span><span style={role}>{solution.role}</span></div><img src={solution.image} alt={solution.name} style={logo}/><h2 style={cardTitle}>{solution.name}</h2><p style={cardBody}>{solution.description}</p><div style={labels}>{solution.labels.map(label=><span key={label} style={labelStyle}>{label}</span>)}</div><span style={explore}>VIEW GOVERNED SOLUTION →</span></Link>)}</div></section>
      <section style={governanceBand}><div style={governanceInner}><div><p style={eyebrow}>// GOVERNANCE</p><h2 style={sectionTitle}>9 core technologies. 5 core systems. 2 specialized solutions.</h2></div><p style={governanceCopy}>Specialized solutions organize approved technologies and components around a specific operating or maintenance context. They do not replace or expand the canonical core taxonomy.</p></div></section>
      <section style={cta}><Link href="/systems/" style={primaryButton}>EXPLORE CORE SYSTEMS</Link><Link href="/knowledge-center/" style={secondaryButton}>KNOWLEDGE CENTER</Link></section>
    </main>
  );
}
