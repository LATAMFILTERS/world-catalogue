        ...
        <div style={grid}>
          {SOLUTIONS.map((solution, index) => (
            <Link key={solution.slug} href={solution.href} style={card}>
              <div style={cardHeader}><span style={number}>{String(index + 1).padStart(2, '0')}</span><span style={role}>{solution.role}</span></div>
              ...
              <div style={labels}>{solution.labels.map((label) => <span key={label} style={labelStyle}>{label}</span>)}</div>
