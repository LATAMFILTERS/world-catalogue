...
const faqs = [
  ['What is an ELIMFILTERS filtration technology?', 'An ELIMFILTERS filtration technology is the engineering architecture assigned to a specific contamination-control function inside a defined protection system. It is not a generic product label and should not be selected independently of the protected system and application.'],
  ['How are systems, technologies, product families and part numbers related?', 'The hierarchy is protected system first, technology second, product family third and validated part number last. This keeps technical selection tied to the function being protected instead of choosing a product only by appearance or cross-reference.'],
  ['How many ELIMFILTERS filtration technologies are presented on this page?', 'Nine technologies are presented here: MACROCORE™, MICROKAPPA™, DRYCORE™, INTEKCORE™, SYNTAPORE™, HYDROCORE™, SYNTRAX™, NANOFORCE™ and THERMACORE™.'],
  ['Which technology is used for engine air intake filtration?', 'MACROCORE™ is the primary ELIMFILTERS technology for engine air-intake filtration. INTEKCORE™ supports the air-cleaner housing architecture, while MICROKAPPA™ and DRYCORE™ serve separate cabin-air and compressed-air functions within the broader airflow protection system.'],
  ['Which technologies are used for diesel fuel protection?', 'SYNTAPORE™ is used for particulate fuel filtration and HYDROCORE™ for approved fuel-water separation applications. These functions are related but are not interchangeable.'],
  ['Which technologies protect lubrication, hydraulic and cooling systems?', 'SYNTRAX™ supports lubrication filtration, NANOFORCE™ supports hydraulic filtration and THERMACORE™ supports cooling-system filtration.'],
  ['Can a technology name alone identify the correct filter?', 'No. The technology identifies the engineering architecture. Final selection still requires the equipment or vehicle, protected system, duty context, product family and validated application evidence.'],
] as const;
...
const heroImage: CSSProperties = { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', opacity: 0.5 };
const heroOverlay: CSSProperties = { position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(0,0,0,.92) 0%, rgba(0,0,0,.70) 54%, rgba(0,0,0,.30) 100%)' };
...
const decisionFlow: CSSProperties = { maxWidth: '1180px', margin: '2rem auto 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '1px', background: 'rgba(255,255,255,0.08)' };
const decisionStep: CSSProperties = { background: '#000', padding: '1.2rem', fontFamily: displayFont, fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.08em', color: '#fff' };
