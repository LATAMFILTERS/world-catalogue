...
const SCHEMA_COVERAGE = [
  { entity: 'Engineering Articles',     total: 53, jsonLd: 'TechArticle',       faqPage: true,  breadcrumb: true,  howTo: false, learningResource: false, dataset: false, eeat: true  },
  { entity: 'Standards',                total: 22, jsonLd: 'TechArticle',       faqPage: false, breadcrumb: false, howTo: false, learningResource: false, dataset: false, eeat: false },
  { entity: 'Glossary Terms',           total: 68, jsonLd: 'DefinedTerm',       faqPage: false, breadcrumb: false, howTo: false, learningResource: false, dataset: false, eeat: false },
  ...
  { entity: 'Datasets (this sprint)',   total: 3,  jsonLd: 'Dataset',           faqPage: false, breadcrumb: true,  howTo: false, learningResource: false, dataset: true,  eeat: false },
];
...
const GEO_SIGNALS = [
  ...
  { signal: 'FAQPage JSON-LD',                   status: 'partial',  detail: '5 priority articles populated with 5–10 FAQs each. 48 articles pending.', since: 'Phase 6G' },
  { signal: 'HowTo JSON-LD',                     status: 'partial',  detail: '2 procedural articles (hydraulic-system-flushing, fleet-oil-sampling-protocol) have full HowTo JSON-LD. 4 more pending.', since: 'Phase 6G' },
  { signal: 'BreadcrumbList JSON-LD',            status: 'complete', detail: 'All engineering articles emit BreadcrumbList with 3-level path', since: 'Phase 6G' },
  { signal: 'Dataset JSON-LD',                   status: 'complete', detail: '3 datasets with DataCatalog + Dataset schema on /knowledge-center/datasets', since: 'Phase 6G' },
  { signal: 'EEAT Metadata Panel (visible)',      status: 'partial',  detail: '5 priority articles populated. Renderer ready for all articles.', since: 'Phase 6G' },
  { signal: 'Engineering References',            status: 'partial',  detail: '4 articles populated with category-tagged references. 49 articles pending.', since: 'Phase 6G' },
  { signal: 'Engineering Decision Guides',       status: 'partial',  detail: '2 articles with interactive decision trees (contamination-control, hydraulic-contamination-sensitivity). 8 more planned.', since: 'Phase 6G' },
  { signal: 'CommonMistakes (expanded)',         status: 'partial',  detail: '5–6 mistakes per populated article. Full expansion pending for 30+ articles.', since: 'Phase 6G' },
  ...
  { signal: 'SearchAction schema (KC hub)',       status: 'pending',  detail: 'SearchAction on /knowledge-center hub page not yet implemented', since: '' },
...
const AI_CITE_CHECKLIST = [
  ...
  { item: 'TechArticle headline + description on all articles',         done: true  },
  { item: 'DefinedTerm schema on all 68 glossary terms',               done: true  },
  { item: 'LearningResource on all 5 learning paths',                  done: true  },
  { item: 'FAQPage on priority articles (5+ per article, 5 articles)',  done: true  },
  ...
  { item: 'EEAT visible metadata on priority articles',                 done: true  },
  { item: 'Engineering References with category tagging',               done: true  },
  { item: 'Authoritative ISO standard citations in every section',      done: true  },
  { item: 'FAQPage on all 53 articles (10 per article)',                done: false },
  { item: 'HowTo on 6 procedural articles',                             done: false },
  { item: 'Decision Guides on 10 articles',                             done: false },
  { item: 'EEAT on all 53 articles',                                    done: false },
  { item: 'FAQPage on all 22 standards',                                done: false },
  { item: 'Product ↔ KC links from /families/[slug]',                  done: false },
  { item: 'SearchAction schema on /knowledge-center hub',               done: false },
];
