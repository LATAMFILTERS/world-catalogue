  ...
  return (
    <main style={{ background: '#000', color: '#fff', minHeight: '100vh', padding: 'clamp(3rem,6vw,5rem) clamp(1.5rem,4vw,4rem)' }}>
      <section style={{ maxWidth: '920px', margin: '0 auto 3rem' }}>
        ...
        <p style={{ color: '#FFF12D', marginTop: '2rem', letterSpacing: '.12em' }}>PROTECTION SYSTEMS</p>
        ...
        <p style={{ color: 'rgba(255,255,255,.68)', lineHeight: 1.75 }}>
          {CANONICAL_PUBLIC_SYSTEMS.length} canonical asset-protection systems form the public ELIMFILTERS architecture. Engine intake, cabin air, compressed-air drying and intake housings are governed within Air Intake &amp; Airflow Protection.
        </p>
      </section>

      <section style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: '1rem' }}>
        {CANONICAL_PUBLIC_SYSTEMS.map((system) => (
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        description: `${CANONICAL_PUBLIC_SYSTEMS.length} canonical industrial asset-protection systems mapped to contamination targets, standards and ELIMFILTERS technologies.`,
        url: 'https://elimfilters.com/knowledge-center/systems/',
        publisher: { '@id': 'https://elimfilters.com/#organization' },
        mainEntity: { '@type': 'ItemList', itemListElement: CANONICAL_PUBLIC_SYSTEMS.map((s, index) => ({ '@type': 'ListItem', position: index + 1, name: s.title, url: `https://elimfilters.com/knowledge-center/systems/${s.slug}/` })) },
      }) }} />
    </main>
  );
}
