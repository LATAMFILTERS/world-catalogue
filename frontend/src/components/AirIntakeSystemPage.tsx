...
const faqs = [
  ['What is Air Intake & Airflow Protection?', 'It is the ELIMFILTERS protection-system domain that organizes engine intake filtration, air-cleaner housings, cabin-air filtration and applicable compressed-air drying around the airflow path they protect.'],
  ['Is cabin-air filtration the same as engine air filtration?', 'No. Both belong to the Air Intake & Airflow Protection architecture, but they protect different airflow paths, use different product families and are evaluated against different application requirements.'],
  ['Which ELIMFILTERS technology protects engine air intake?', 'MACROCORE™ is the primary ELIMFILTERS technology for engine air-intake filtration within this protection system.'],
  ['What role does INTEKCORE™ play?', 'INTEKCORE™ is associated with air-cleaner housings and the physical airflow architecture that supports sealing, routing and filter installation.'],
  ['Where do MICROKAPPA™ and DRYCORE™ fit?', 'MICROKAPPA™ supports cabin-air filtration. DRYCORE™ supports air-dryer filtration in applicable compressed-air systems. They are supporting technologies inside the broader Air Intake & Airflow Protection domain.'],
  ['How should an air filter be selected?', 'Selection should follow the equipment or vehicle, engine or protected airflow path, housing configuration, duty environment, dimensions and validated application or OEM evidence.'],
  ['Can one air filter be selected only by dimensions?', 'Dimensions are useful evidence but are not sufficient by themselves. Sealing geometry, airflow requirement, housing fit, duty environment and application compatibility also matter.'],
] as const;
    ...
    <main style={main}>
      ...
      <section style={section}>
        ...
        <div style={decisionFlow}>
          {['1. EQUIPMENT / VEHICLE', '2. PROTECTED AIRFLOW PATH', '3. HOUSING + SEAL CONFIGURATION', '4. DUTY ENVIRONMENT', '5. DIMENSIONS + OEM EVIDENCE', '6. VALIDATED ELIMFILTERS PART'].map((step) => (
...
const grid4: CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '1px', background: 'rgba(255,255,255,.08)', marginTop: '2rem' };
const grid3: CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1px', background: 'rgba(255,255,255,.08)', marginTop: '2rem' };
...
const lineCard: CSSProperties = { background: '#000', padding: '1.6rem 1.8rem', borderLeft: '2px solid rgba(255,241,45,.55)' };
...
const familyGrid: CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1px', background: 'rgba(255,255,255,.08)', marginTop: '2rem' };
const familyCard: CSSProperties = { background: '#050505', padding: '1.6rem', textDecoration: 'none', color: '#fff', display: 'block' };
...
const decisionFlow: CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '1px', marginTop: '2rem', background: 'rgba(255,255,255,.08)' };
const decisionStep: CSSProperties = { background: '#050505', padding: '1.2rem', fontFamily: displayFont, fontWeight: 700, fontSize: '.74rem', letterSpacing: '.08em' };
