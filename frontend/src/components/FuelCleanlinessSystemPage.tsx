...
const faqs = [
  ['What is Fuel Cleanliness Protection?', 'It is the ELIMFILTERS protection-system architecture for controlling particulate contamination and water before diesel fuel reaches sensitive pumps, injectors and downstream fuel components.'],
  ['Which ELIMFILTERS technologies belong to Fuel Cleanliness Protection?', 'SYNTAPORE™ supports particulate fuel filtration, HYDROCORE™ supports fuel/water separation, and TURBOCORE™ supports applicable turbine-series fuel-separation systems.'],
  ['Is every fuel filter a fuel/water separator?', 'No. Particulate filtration and fuel/water separation are different functions. The product family and application must confirm whether water separation is required.'],
  ['Why does stored fuel matter?', 'Fuel can accumulate water, sediment and transfer contamination before it reaches the engine. Storage and handling conditions therefore form part of the contamination path.'],
  ['Can a fuel filter be selected only from a cross reference?', 'A cross reference is evidence, not the entire application decision. Engine, fuel-system configuration, filtration stage, separator function, dimensions and validated application evidence should agree.'],
  ['What information is useful for fuel-filter identification?', 'Vehicle or equipment, engine, fuel-system stage, existing filter or OEM number, dimensions, separator configuration and any known service conditions are useful inputs.'],
  ['Where does the turbine-series product line fit?', 'Applicable FH / FG turbine-series fuel-separation systems belong to Fuel Cleanliness Protection and use TURBOCORE™ replacement elements within that specific product architecture.'],
] as const;
    ...
    <main style={main}>
      ...
      <section style={section}>
        ...
        <div style={decisionFlow}>
          {['1. EQUIPMENT / VEHICLE', '2. ENGINE + FUEL SYSTEM', '3. FILTRATION STAGE', '4. WATER-SEPARATION REQUIREMENT', '5. OEM / FILTER EVIDENCE', '6. VALIDATED ELIMFILTERS PART'].map((step) => (
...
const grid3: CSSProperties = { maxWidth: '1180px', margin: '2.4rem auto 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: '1px', background: 'rgba(255,255,255,.08)' };
const grid2: CSSProperties = { maxWidth: '1180px', margin: '2.4rem auto 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: '1px', background: 'rgba(255,255,255,.07)' };
const card: CSSProperties = { background: '#000', padding: '1.6rem' };
const lineCard: CSSProperties = { background: '#000', padding: '1.7rem' };
...
const familyGrid: CSSProperties = { maxWidth: '1180px', margin: '2.4rem auto 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: '1px', background: 'rgba(255,255,255,.07)' };
const familyCard: CSSProperties = { background: '#000', padding: '1.6rem', textDecoration: 'none', color: '#fff', display: 'block' };
...
const decisionFlow: CSSProperties = { maxWidth: '1180px', margin: '2.2rem auto 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '1px', background: 'rgba(255,255,255,.08)' };
const decisionStep: CSSProperties = { background: '#000', padding: '1.25rem', fontFamily: displayFont, fontSize: '.72rem', fontWeight: 700, letterSpacing: '.08em', color: 'rgba(255,255,255,.72)' };
