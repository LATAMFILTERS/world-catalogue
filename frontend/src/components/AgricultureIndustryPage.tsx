    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Agriculture', item: PAGE_URL },
        ],
      },
      {
        '@type': 'ItemList',
    ...
    <main id="main-content" style={main}>
      ...
      <section style={hero}>
        <video autoPlay muted loop playsInline preload="auto" style={heroVideo}>
          <source src="/images/Agriculture-2.mp4" type="video/mp4" />
        </video>
        <div style={heroShade} />
        <div style={heroInner}>
          ...
          <div style={actions}>
            <Link href="/contact/" data-conversion-action="application-support" style={primaryButton}>PROTECT AGRICULTURAL EQUIPMENT</Link>
      ...
      <section style={seasonSection} aria-labelledby="season-title">
        <div style={contentWidth}>
          <p style={eyebrow}>SEASONAL UPTIME</p>
          <h2 id="season-title" style={sectionTitle}>The protection strategy changes with the work cycle.</h2>
          <p style={wideText}>Agriculture concentrates risk into short operating windows. A filter decision that is acceptable during light service may be inadequate once dust loading, crop residue, heat and operating hours increase together.</p>
          <div style={seasonGrid}>
            {seasonalWindows.map(([title, text]) => <article key={title} style={seasonCard}><h3 style={cardTitle}>{title}</h3><p style={bodyText}>{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section style={equipmentSection} aria-labelledby="equipment-title">
        <div style={equipmentGrid}>
          <div style={equipmentMedia}><img src="/images/agriculture-2_converted.avif" alt="Agricultural equipment operating in field conditions" style={equipmentImage} /></div>
          <div style={equipmentCopy}>
            <h2 id="equipment-title" style={sectionTitle}>Different machines. Different field exposure.</h2>
            <p style={wideText}>A tractor, combine, sprayer and irrigation engine may work on the same farm but face very different contamination loads, service access and operating intensity.</p>
            <div role="list" aria-label="Agricultural equipment applications" style={equipmentList}>
              {equipment.map((asset) => <div role="listitem" key={asset} style={equipmentRow}><span style={equipmentName}>{asset}</span></div>)}
            </div>
          </div>
        </div>
      </section>

      <section style={section} aria-labelledby="exposure-title">
        <div style={contentWidth}>
          <p style={eyebrow}>FIELD EXPOSURE</p>
          <h2 id="exposure-title" style={sectionTitle}>Five contamination pathways shape agricultural reliability.</h2>
          <p style={wideText}>The useful question is not simply which filter fits the machine. It is where contamination enters, which system is exposed and what component must remain protected through the seasonal duty cycle.</p>
          <div style={pathwayGrid}>
            {protection.map(([source, detail, system, technology, href]) => (
              <article key={system} style={pathwayCard}>
                <div><span style={smallLabel}>CONTAMINATION SOURCE</span><strong style={pathwaySource}>{source}</strong><p style={smallText}>{detail}</p></div>
                <div style={pathwayDivider} />
                <div><span style={smallLabel}>PROTECTION SYSTEM</span><strong style={pathwaySystem}>{system}</strong></div>
                <Link href={href} style={technologyLink}><span style={smallLabelYellow}>ELIMFILTERS TECHNOLOGY</span><strong style={technologyName}>{technology}</strong><span style={technologyCta}>EXPLORE →</span></Link>
              </article>
            ))}
          </div>
        </div>
      ...
      <section style={principleSection}>
        <div style={principle}>
          <span style={principleLabel}>AGRICULTURE ENGINEERING PRINCIPLE</span>
      ...
      <section style={continuitySection} aria-labelledby="continuity-title">
        <div style={contentWidth}>
          <p style={eyebrow}>FIELD CONTINUITY</p>
          <h2 id="continuity-title" style={sectionTitle}>When the season is moving, maintenance flexibility disappears.</h2>
          <div style={continuityGrid}>
            {[
            ...
            ].map(([title, text]) => <article key={title} style={continuityCard}><h3 style={continuityTitle}>{title}</h3><p style={bodyText}>{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section style={section} aria-labelledby="decision-title">
        <div style={contentWidth}>
          ...
          <h2 id="decision-title" style={sectionTitle}>Resolve the application in four decisions.</h2>
          <div style={decisionGrid}>
            {[
              ['Machine', 'Identify the tractor, combine, sprayer, engine or support asset.'],
              ...
              ['Product Evidence', 'Resolve the final filter through OEM reference, dimensions and validated application data.'],
            ].map(([title, text]) => <article key={title} style={decisionCard}><h3 style={cardTitle}>{title}</h3><p style={bodyText}>{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section style={faqSection} aria-labelledby="faq-title">
        <div style={contentWidth}>
          <p style={eyebrow}>AGRICULTURAL FILTRATION QUESTIONS</p>
          <h2 id="faq-title" style={sectionTitle}>What operators and maintenance teams need to know.</h2>
      ...
      <section style={conversionSection}>
        <div style={conversionGrid}>
          <div><p style={eyebrow}>HAVE A MACHINE OR FIELD-DUTY PROBLEM?</p><h2 style={conversionTitle}>Build the filtration path around the season, not just the part number.</h2><p style={conversionText}>Send the machine, engine, protected system, field conditions and any known OEM or filter reference.</p><Link href="/contact/" data-conversion-action="application-support" style={primaryButton}>REQUEST APPLICATION SUPPORT</Link></div>
...
const sectionTitle: CSSProperties = { fontFamily: displayFont, fontWeight: 700, fontSize: 'clamp(2.2rem,4.7vw,4.2rem)', lineHeight: .96, letterSpacing: '-.04em', textTransform: 'uppercase', margin: 0, maxWidth: '900px' };
...
const equipmentSection: CSSProperties = { padding: 'clamp(4.5rem,8vw,7rem) clamp(1.25rem,6vw,6rem)', background: '#050505', borderBottom: '1px solid rgba(255,255,255,.08)' };
...
const equipmentLead: CSSProperties = { ...wideText, maxWidth: '600px' };
...
const decisionGrid: CSSProperties = { marginTop: '2.6rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(240px,100%),1fr))', gap: '1px', background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.1)' };
const decisionCard: CSSProperties = { background: '#030303', minHeight: '190px', padding: '1.5rem' };
