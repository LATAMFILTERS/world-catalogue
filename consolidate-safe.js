...
async function consolidateWithProgressiveDelete() {
    ...
    try {
        ...
        for (const nombreCol of coleccionesOrigen) {
            try {
                ...
                if (total === 0) {
                    console.log(`⏭️  ${nombreCol} ya procesada\n`);
                    continue;
                }
                ...
                while (true) {
                    // Tomar documentos en lotes pequeños
                    const docs = await origen.find().limit(50).toArray();
                    
                    if (docs.length === 0) {
                        console.log(`  ✅ ${nombreCol} COMPLETADA`);
                        break;
                    }
                    ...
                    if (batch.length > 0) {
                        try {
                            ...
                            if (err.code === 11000 && err.result?.nInserted) {
                                totalGuardados += err.result.nInserted;
                            }
                        }
                    ...
                    if (procesados % 500 === 0) {
                        ...
                        console.log(`  ✅ ${procesados} procesados | ${restantes} restantes`);
                    }
                }
                ...
                if (restantes === 0) {
                    await origen.drop();
                    console.log(`  🗑️  ${nombreCol} ELIMINADA\n`);
                } else {
                    console.log(`  ⚠️  ${nombreCol} tiene ${restantes} docs sin procesar\n`);
                }
                
                totalProcesados += procesados;
                
            } catch (err) {
                console.error(`❌ Error en ${nombreCol}:`, err.message);
                console.log('Continuando con siguiente colección...\n');
            }
        }
        ...
        console.log(`📈 SUPER_MAESTRO: ${finalCount} documentos`);
...
consolidateWithProgressiveDelete();
