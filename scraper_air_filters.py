...
AIR_FILTERS_URL = "https://shop.donaldson.com/store/en-us/search?N=2975800598&Nr=product.language%3AEnglish&catNav=true&st=parts"
...
def run_donaldson_air_filters():
    with sync_playwright() as p:
        user_data_dir = os.path.join(os.getcwd(), "sesion_air_filters")
        ...
        while True:
            ...
            if has_next:
                print(f"\n{'='*80}")
                print(f"📄 Página {page_num} completada. Total: {total_processed}")
                print(f"{'='*80}")
        ...
        print(f"\n{'='*80}")
        print(f"🏆 AIR FILTERS SCRAPING COMPLETADO")
        print(f"{'='*80}")
        print(f"Total procesados: {total_processed}")
        print(f"Exitosos: {successful}")
        print(f"Errores: {len(results) - successful}")

        with open("air_filters_results.json", "w", encoding="utf-8") as f:
            json.dump(results, f, indent=4, ensure_ascii=False)

        print(f"\n📁 Guardado en: air_filters_results.json")
        context.close()

if __name__ == "__main__":
    run_donaldson_air_filters()
