...
function ticketEmail(ticket) {
  ...
  // Present only when this ticket came from the support-lead flow -- this
  // is the customer's own email, shown here purely so the internal team
  // can reply to them. The department_email above (this notification's
  // recipient) is always the internal support address and never the
  // customer's -- one never substitutes for the other.
  if (ticket.email) rows.push(['Customer email', ticket.email]);
  if (ticket.language) rows.push(['Language', ticket.language]);
