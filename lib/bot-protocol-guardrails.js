...
function inferResolutionStatus(payload = {}) {
  if (payload?.diagnostic?.complete && payload?.evidence?.validated) return 'resolved_with_catalog_evidence';
  ...
  if (payload?.diagnostic?.missing_field) return 'awaiting_customer_data';
  return 'unresolved';
}
...
function applyProtocolGuardrails(payload = {}, requestBody = {}) {
  const context = requestBody?.context && typeof requestBody.context === 'object'
    ? requestBody.context
    : {};
  const attempts = Math.max(0, Number(context.unresolved_attempts || context.attempts || 0));
