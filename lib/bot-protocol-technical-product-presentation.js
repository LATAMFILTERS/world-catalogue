...
const BRAND_ALIASES = new Map([
  ['CAT', 'Caterpillar'], ['CATERPILLAR', 'Caterpillar'], ['CUMMINS', 'Cummins'],
  ['VOLVO', 'Volvo'], ['JOHNDEERE', 'John Deere'], ['JOHN DEERE', 'John Deere'],
  ['MACK', 'Mack'], ['DETROIT', 'Detroit Diesel'], ['DETROITDIESEL', 'Detroit Diesel'],
  ['MERCEDESBENZ', 'Mercedes-Benz'], ['SCANIA', 'Scania'], ['HINO', 'Hino'],
  ['ISUZU', 'Isuzu'], ['TOYOTA', 'Toyota'], ['KOMATSU', 'Komatsu'], ['HITACHI', 'Hitachi'],
  ['CASE', 'CASE'], ['NEWHOLLAND', 'New Holland'], ['AGCO', 'AGCO'], ['KENWORTH', 'Kenworth'],
  ['PETERBILT', 'Peterbilt'], ['FREIGHTLINER', 'Freightliner'], ['INTERNATIONAL', 'International'],
  ['FLEETGUARD', 'Fleetguard'], ['DONALDSON', 'Donaldson'], ['BALDWIN', 'Baldwin'],
  ['WIX', 'WIX'], ['MANN', 'MANN'], ['MANNFILTER', 'MANN-FILTER'], ['MAHLE', 'MAHLE'],
  ['BOSCH', 'Bosch'], ['FRAM', 'FRAM']
]);
...
function uniqueReferenceRows(rows) {
  ...
  for (const row of rows || []) {
    const manufacturer = canonicalBrand(row?.manufacturer || row?.brand);
    const code = clean(row?.code || row?.reference, 80);
    if (!manufacturer || !code) continue;
    const key = `${manufacturer.toUpperCase()}|${normalizeReference(code)}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({ manufacturer, code, score: Number(row?.score || 0), status: row?.status || null });
  }
  return out;
}
...
function selectEquivalentReferences(product = {}, referenceSource = {}, max = 3) {
  ...
  const rows = uniqueReferenceRows(product.protocol_validated_crossrefs || []).filter(row => {
  ...
  const oemRows = rows
    ...
    .sort((a, b) => priorityIndex(a.manufacturer, OEM_PRIORITY) - priorityIndex(b.manufacturer, OEM_PRIORITY) || b.score - a.score);
  ...
  const aftermarketRows = rows
    ...
    .sort((a, b) => priorityIndex(a.manufacturer, AFTERMARKET_PRIORITY) - priorityIndex(b.manufacturer, AFTERMARKET_PRIORITY) || b.score - a.score);

  const selected = [];
  const manufacturers = new Set();
  for (const row of [...oemRows, ...aftermarketRows, ...rows.sort((a, b) => b.score - a.score)]) {
...
function technicalDescription(product = {}, language = 'es') {
  ...
  if (raw) {
    return raw.replace(/^ELIMFILTERS®?\s*/i, '').replace(/\s+/g, ' ').trim();
  }
...
function technicalAnswer(product, referenceSource, applications, equivalents, language = 'es') {
  ...
  if (language === 'en') {
    ...
    if (equivalents.length) lines.push(`Primary validated cross-references: ${equivalents.map(item => `${item.manufacturer} ${item.code}`).join('; ')}.`);
    return lines.join('\n');
  }
  ...
  const lines = [
    ...
    `SKU ELIMFILTERS: ${sku}.`,
    `Clasificación del filtro: ${type}.`,
    `Descripción técnica: ${description}`
  ];
  if (applications.length) lines.push(`Aplicaciones principales validadas: ${applications.join('; ')}.`);
  if (equivalents.length) lines.push(`Cross-references validados principales: ${equivalents.map(item => `${item.manufacturer} ${item.code}`).join('; ')}.`);
  return lines.join('\n');
}
...
function applyTechnicalProductPresentation(payload = {}, requestBody = {}) {
  ...
  return {
    ...
    governance: {
      ...(governed.governance || {}),
      technical_presentation: 'validated_deterministic',
