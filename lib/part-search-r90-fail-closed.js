...
if (!express.application.__elimR90FailClosedPatch) {
  ...
  express.application.get = function patchedR90Get(path, ...handlers) {
    ...
    const governR90 = async (req, res, next) => {
      ...
      // TECNOCAR R90 -> EL84004. R90S/R90T/R90P remain independent RACOR
      ...
      try {
        ...
        return res.status(200).json({
          success: true,
          source: bareValidatedR90 ? 'validated_reference_exact' : 'manufacturer_reference_exact',
          resolution: 'RESOLVED',
          results: [product],
          ...
          duty_resolution: bareValidatedR90 ? 'VALIDATED_REFERENCE_EXACT' : 'MANUFACTURER_REFERENCE_EXACT',
...
console.log('[part-search-r90] validated R90 and TECNOCAR R90 resolve directly to EL84004');
