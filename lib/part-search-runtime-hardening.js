...
// server-original historically installed statement_timeout with an async
// pool "connect" listener. A newly-created pg.Client can be handed to the
// request while that SET query is still active, which triggers pg's
// "client.query() when the client is already executing a query" deprecation
// warning. Inject the same timeout through pg connection configuration and
// suppress only that obsolete initialization listener.
...
function patchQueryable(proto) {
  ...
  proto.query = function patchedQuery(config, values, callback) {
    ...
    // A part-number-shaped token should be resolved only by SKU/base/reference
    // authority. Searching free-text descriptions for that token cannot prove
    // a cross-reference and was the dominant ~1s negative-path cost. Skip only
    // this final descriptive fallback; exact, canonical, xref, prefix and duty
    // governance remain untouched and fail-closed.
...
console.log('[part-search-runtime] strict certified canonical PART NUMBER guard + normalized lookup + pg serialization hardening enabled');
