...
const DEFAULT_STATEMENT_TIMEOUT_MS = 8000;
const OriginalPool = pg.Pool;

class HardenedPool extends OriginalPool {
  constructor(config = {}) {
    super({
      ...config,
      statement_timeout: config.statement_timeout ?? DEFAULT_STATEMENT_TIMEOUT_MS,
    });
  }
  ...
  on(eventName, listener) {
    if (
      ...
      /SET\s+statement_timeout/i.test(Function.prototype.toString.call(listener))
    ) {
      return this;
    }
...
function isDescriptionFullTextQuery(sql) {
  return (
    typeof sql === 'string' &&
    /to_tsvector\s*\(\s*'english'\s*,\s*COALESCE\s*\(\s*description::text/i.test(sql)
  );
}

function emptySelectResult() {
  return {
    command: 'SELECT',
    rowCount: 0,
    oid: null,
...
function patchQueryable(proto) {
  ...
  proto.query = function patchedQuery(config, values, callback) {
    const originalText = typeof config === 'string'
      ? config
      : (config && typeof config === 'object' ? config.text : '');
    const queryValues = Array.isArray(values)
      ? values
      : (config && typeof config === 'object' && Array.isArray(config.values) ? config.values : []);

    if (isDescriptionFullTextQuery(originalText) && looksLikePartNumber(queryValues[0])) {
      const result = emptySelectResult();
      const cb = typeof callback === 'function'
        ? callback
        : (typeof values === 'function' ? values : null);
      if (cb) {
...
function rowSku(row) {
  ...
  return String(row.sku || row.elimfilters_sku || row.elim_code || '').toUpperCase();
}

function isElimSkuString(value) {
  return /^E[A-Z][A-Z0-9-]{4,}$/i.test(String(value || '').trim());
}

function isProductCollectionKey(key) {
  return /^(results|candidates|products|alternatives|alternative_products|related|related_products|recommendations|recommended|recommended_products|matches|match_products|equivalents|substitutes)$/i.test(String(key || ''));
}

function sanitizeExactPartCollections(value, allowedSku, keyName = '') {
  if (Array.isArray(value)) {
    let next = value;
    if (isProductCollectionKey(keyName)) {
      next = value.filter((entry) => {
        if (typeof entry === 'string') {
          return !isElimSkuString(entry) || entry.toUpperCase() === allowedSku;
        }
        const sku = rowSku(entry);
        return !sku || sku === allowedSku;
      });
    }
    return next.map((entry) => sanitizeExactPartCollections(entry, allowedSku, ''));
  }

  if (!value || typeof value !== 'object') return value;
  const copy = { ...value };
  for (const [key, child] of Object.entries(copy)) {
    copy[key] = sanitizeExactPartCollections(child, allowedSku, key);
  }
  return copy;
}
...
function filterResolvedSku(body, allowedSku) {
  ...
  const canonicalSku = String(allowedSku).toUpperCase();
  const copy = sanitizeExactPartCollections(body, canonicalSku);

  // Exact part-number mode has one certified canonical identity. Never expose
  // another ELIMFILTERS SKU as an "alternative", "related", "match" or
  // substitute merely because it shares secondary OEM/competitor references.
  // This prevents cross-family leakage such as a conventional fuel filter
  // being presented next to a validated fuel/water separator.
  copy.exact_part_isolation = true;
  copy.resolved_sku = copy.resolved_sku || canonicalSku;
  copy.alternative_resolution_policy = 'CERTIFIED_CANONICAL_ONLY';
  return copy;
}
    ...
    const audit = async (req, res, next) => {
      ...
      try {
        if (strictPartMode && q) {
          ...
          allowedSku = String(skus[0]).toUpperCase();
        }
      } catch (error) {
...
console.log('[part-search-runtime] strict certified canonical PART NUMBER guard + exact-reference family isolation + normalized lookup + pg serialization hardening enabled');
