...
function isPartNumberLike(value) {
  const ref = normalizeValue(value);
  if (ref.length < 4 || ref.length > 32) return false;
  // ELIMFILTERS/OEM/competitor part numbers in this catalog are numeric or
  // alphanumeric. Pure alphabetic tokens are overwhelmingly manufacturer,
  // field-label or metadata contamination and must not define a reference family.
...
module.exports = { normalizeValue, unique, isPartNumberLike, classifyReferenceFamily, singleDuty };
