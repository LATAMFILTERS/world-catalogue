...
const RECOMMENDATION_PATTERN = /(?:qu[eé]|cu[aá]l)\s+filtro|filtro\s+recomienda|recomi[eé]ndame\s+(?:un\s+)?filtro|qu[eé]\s+referencia/i;
...
function isRecommendationRequest(message) {
  return RECOMMENDATION_PATTERN.test(String(message || '').trim());
}
...
function hasExactEquipmentContext(body = {}) {
  const context = body.context && typeof body.context === 'object' ? body.context : {};
  const combined = conversationText(body);
  const tokens = Array.isArray(context.equipment_tokens) ? context.equipment_tokens : [];
  const hasBrand = /\b(?:MACK|VOLVO|FREIGHTLINER|KENWORTH|PETERBILT|CUMMINS|DETROIT|JOHN\s*DEERE)\b/i.test(combined) || tokens.length >= 1;
  const hasModelOrEngine = /\b(?:MP\d|DD\d+|SERIES\s*60|ISX\d*|X15|\d{3,4}\s*(?:CTS|STS)|[A-Z]{1,4}\d{2,5})\b/i.test(combined) || tokens.length >= 2;
  return hasBrand && hasModelOrEngine;
}
...
function recommendationResponse(body = {}, payload = {}) {
  if (payload?.intent === 'application_lookup' && (
    payload?.phase === 'collecting_application_data' ||
    payload?.evidence?.validated ||
    payload?.phase === 'catalog_resolution'
  )) {
    return payload;
  }

  const context = body.context && typeof body.context === 'object' ? body.context : {};
  const exactEquipment = hasExactEquipmentContext(body);

  return {
    ...payload,
    protocol_version: payload?.protocol_version || '1.2.1',
    intent: 'application_lookup',
    phase: exactEquipment ? 'catalog_resolution_required' : 'collecting_equipment_data',
    diagnostic: {
      ...(payload?.diagnostic || {}),
      equipment_tokens: context.equipment_tokens || payload?.entities?.equipment_tokens || [],
      complete: false,
      missing_field: exactEquipment ? 'catalog_validation' : 'equipment'
    },
    ...
    answer: exactEquipment
      ? 'No voy a recomendar un filtro sin una aplicación confirmada en el catálogo ELIMFILTERS. Indica la referencia del filtro instalado o confirma marca, modelo, motor y año completos para validar el SKU exacto.'
      : 'Para recomendar el filtro correcto necesito identificar el equipo. Indica marca, modelo, motor y año, o la referencia del filtro instalado. No asignaré un SKU sin evidencia en el catálogo ELIMFILTERS.'
  };
}
...
function installDiagnosticOverride(app) {
  app.use('/api/bot/protocol', (req, res, next) => {
    ...
    const recommendationRequest = isRecommendationRequest(req.body?.message);
    if (req.method !== 'POST' || (!isDiagnosticMessage(req.body?.message) && !continuingDiagnostic && !recommendationRequest)) return next();

    const originalJson = res.json.bind(res);
    res.json = payload => {
      if (recommendationRequest) return originalJson(recommendationResponse(req.body || {}, payload));
