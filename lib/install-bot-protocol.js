...
const { installCatalogReferenceGovernance } = require('./part-search-reference-governance-patch');
const { installHermesReviewCenter } = require('./hermes-review-center');

// IMPORTANT: install the catalog wrapper before the web adapter imports the
// unified orchestrator. The orchestrator destructures searchByReferences at
// module load, so governance must be active first for every deterministic
// reference lookup.
