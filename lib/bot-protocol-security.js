...
function installProtocolSecurity(app) {
  app.use('/api/bot/protocol', (req, res, next) => {
    ...
    const isImageRequest = req.path === '/image' || req.originalUrl?.split('?')[0] === '/api/bot/protocol/image';
    const validationError = validateBody(req.body, { image: isImageRequest });
    ...
    if (!isImageRequest) req.body.message = req.body.message.trim();
