...
function actionableDeepLink(value) {
...
function createJobNotifier(options = {}) {
  ...
  const deepLink = actionableDeepLink(options.deepLink);
  ...
  async function publish(type, title, message, context = {}) {
    ...
    return jobEvent(normalized, module, title || baseTitle, message, {
      ...
      entityType,
      entityId: jobId,
      deepLink: actionableDeepLink(context.deepLink) || deepLink,
  ...
  return {
    ...
    checkpoint(marker, message, context = {}) {
      const key = String(marker || '').trim().toUpperCase();
      if (!key || emittedCheckpoints.has(key)) return Promise.resolve(null);
      emittedCheckpoints.add(key);
      return publish('JOB_PROGRESS', context.title || baseTitle, message, { ...context, marker: key, metadata: { checkpoint: key, ...(context.metadata || {}) } });
    },
...
module.exports = { createJobNotifier, createRunId, actionableDeepLink };
