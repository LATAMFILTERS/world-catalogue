...
// ============================================================================
// CRITICAL FIX: JSONB Array Handling in nvidia.js Catalog Lookup
// ============================================================================
/*
ISSUE: Bots must correctly identify OEM/competitor codes that CLIENTS know
(e.g., "Donaldson P552100") and return the equivalent ELIMFILTERS SKU.

DATABASE STRUCTURE:
- oem_codes: JSONB array of objects
  Example: [{"manufacturer": "BALDWIN", "code": "B495"}, {"manufacturer": "DONALDSON", "code": "P552100"}]
- competitor_codes: JSONB array of objects
  Example: [{"manufacturer": "FLEETGUARD", "code": "LF3620"}, {"manufacturer": "WIX", "code": "33212"}]

CORRECT PATTERN (fixed in LinkedIn bot):
When searching for a code in the JSONB arrays, use:

// For OEM codes
const matchedOemBrand = (Array.isArray(res.rows[0].oem_codes) ? res.rows[0].oem_codes : [])
  .find(item => item.code && item.code.toUpperCase().includes(searchTerm))?.manufacturer || 'OEM';

// For competitor codes
const matchedCompetitorBrand = (Array.isArray(res.rows[0].competitor_codes) ? res.rows[0].competitor_codes : [])
  .find(item => item.code && item.code.toUpperCase().includes(searchTerm))?.manufacturer || 'Cross-reference';

INCORRECT PATTERN (DO NOT USE):
const matchedOemBrand = Object.entries(res.rows[0].oem_codes || {})
  .find(([brand, code]) => code && code.toUpperCase().includes(searchTerm))?.[0];
// ^ This assumes {brand: code} object structure, but actual DB has array of objects

EXPECTED RESPONSE FORMAT:
When user says: "Donaldson P552100"
Bot responds: "✓ Tu código Donaldson P552100 equivale a SKU ELIMFILTERS: EA10695 (HEAVY_DUTY)"

This format requires:
1. Extracting the matched manufacturer from array element: item.manufacturer
2. Including the searched code in response: searchTerm
3. Including the ELIMFILTERS SKU: sku
4. Including the duty classification: duty

IMPLEMENTATION REFERENCE:
See elimfilters-linkedin-bot/src/nvidia.js lines 38-96 for working implementation
Commit: 62d7a5e6db (fix: Correct JSONB array handling for oem_codes and competitor_codes)
*/
