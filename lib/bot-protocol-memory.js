...
const { isStandaloneGreeting } = require('./bot-protocol-greeting');
...
function memoryFromResponse(previous, body, payload) {
  ...
  const isGreeting = payload?.intent === 'greeting';
  ...
  return normalizeMemory({
    ...
    equipment_tokens: isGreeting ? [] : diagnostic.equipment_tokens?.length
    ...
    symptoms: isGreeting ? [] : diagnostic.symptoms?.length ? diagnostic.symptoms : previous.symptoms,
    duration: isGreeting ? null : diagnostic.duration || previous.duration,
    operating_context: isGreeting ? null : diagnostic.operating_context || previous.operating_context,
    impact: isGreeting ? null : diagnostic.impact || previous.impact,
    pending_field: isGreeting ? null : diagnostic.missing_field || null,
    current_filter: isGreeting ? null : diagnostic.current_filter ?? previous.current_filter,
    current_filter_references: isGreeting ? [] : diagnostic.current_filter_references?.length
      ? diagnostic.current_filter_references
      : previous.current_filter_references,
    current_filter_status: isGreeting ? null : diagnostic.current_filter_status || previous.current_filter_status,
    unresolved_attempts: isGreeting ? 0 : unresolved ? previous.unresolved_attempts + 1 : 0,
    active_intent: isGreeting ? null : payload?.intent || previous.active_intent
  });
}
...
function installProtocolMemory(app) {
  app.use('/api/bot/protocol', async (req, res, next) => {
    try {
      ...
      const greeting = isStandaloneGreeting(req.body?.message);
      const stored = greeting || isNewDiagnosticStart(req.body?.message, persisted)
        ? resetDiagnosticMemory(persisted)
        : persisted;
      const supplied = greeting
        ? {}
