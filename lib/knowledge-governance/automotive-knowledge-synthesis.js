...
function sourceEvidenceFromProfile(profile) {
  return {
    ...
    source_publisher: profile.source_publisher,
...
function metricsForSeed(seed, profilesById) {
  const metrics = seed.source_ids.flatMap((id) => profilesById.get(id)?.metrics || []);
  return uniqueObjects(metrics, (m) => `${m.name}|${m.value}|${m.unit || ''}|${m.source_id}`);
}
...
function synthesizeAutomotiveKnowledge(profiles = []) {
  ...
  for (const seed of AUTOMOTIVE_KNOWLEDGE_SEEDS) {
    ...
    output.push({
      ...
      source_ids_resolved: sourceEvidence.map((e) => e.source_id),
...
function buildSharedEngineeringFromCanonical(canonicalRecords = []) {
  ...
  return [...byConcept.entries()].map(([concept, objects]) => {
    const evidence = uniqueObjects(objects.flatMap((o) => o.source_evidence || []), (e) => e.source_id);
    const relationships = uniqueObjects(objects.flatMap((o) => o.technical_relationships || []), (r) => r.statement).slice(0, 20);
    const metrics = uniqueObjects(objects.flatMap((o) => o.metrics || []), (m) => `${m.name}|${m.value}|${m.unit || ''}|${m.source_id}`).slice(0, 30);
