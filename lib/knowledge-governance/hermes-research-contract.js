...
// Domain context is descriptive routing metadata only; it does not grant
// publication, technology, application or SKU authority.
function createHermesResearchRequest(input = {}) {
  return {
    ...
    applications: normalizeApplications(input.applications),
...
function validateHermesResearchRequest(request = {}) {
  ...
  if (request.knowledge_domain != null) {
    if (!isValidDomain(request.knowledge_domain)) {
      errors.push('knowledge_domain is invalid');
    } else {
      if (request.system && !isSystemAllowedForDomain(request.knowledge_domain, request.system)) {
        errors.push('system is not allowed for knowledge_domain');
      }
      if (request.industry && !isIndustryAllowedForDomain(request.knowledge_domain, request.industry)) {
        errors.push('industry is not allowed for knowledge_domain');
      }
      if (request.knowledge_domain === KNOWLEDGE_DOMAINS.SHARED && request.industry != null) {
        errors.push('shared engineering research must not be owned by a single industry');
      }
    }
  }

  if (!VALID_TECHNOLOGY_RELATIONS.includes(request.technology_relation)) {
    errors.push('technology_relation is invalid');
  }
  if (!VALID_APPLICATION_RELATIONS.includes(request.application_relation)) {
    errors.push('application_relation is invalid');
  }
  if (request.technology_relation === 'confirmed' && !request.technology) {
    errors.push('confirmed technology_relation requires technology');
  }
