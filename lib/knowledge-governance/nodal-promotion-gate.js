...
function renderApprovedCanonicalNote(markdown, review) {
  ...
  text = text.replace('## Metrics — Awaiting Validation', '## Metrics — Canonical');
  text = text.replace('## Technical Relationships — Awaiting Validation', '## Technical Relationships — Validated');
  if (review.scope_review?.technology_relation_resolution === 'confirmed') text = text.replace(/- Technology relation: [^\n]+/, '- Technology relation: confirmed');
  if (review.scope_review?.application_relation_resolution) text = text.replace(/- Application relation: [^\n]+/, `- Application relation: ${review.scope_review.application_relation_resolution}`);
