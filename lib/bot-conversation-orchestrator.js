...
async function runBotProtocol(body) {
  ...
  const payload = buildPayload({ requestId, state, answer, classifierSource, catalog, knowledge, classification });
  return finalize({ requestId, key, state, body, payload, memorySource });
}

function buildPayload({ requestId, state, answer, classifierSource, catalog, knowledge, classification }) {
  return {
