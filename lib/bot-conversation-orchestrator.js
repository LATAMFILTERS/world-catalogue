...
const { formatForChannel, detectLanguage, copyFor, LANGUAGE_NAMES } = require('./bot-protocol-channel-format');
...
function directAnswer(intent, state, language = 'en') {
  if (intent === 'distribution_inquiry') {
    if (state && state.distributionQualificationConfirmed) {
      return copyFor(language, 'distributionConfirmed');
    }
    return copyFor(language, 'distributionAsk');
  }
  if (intent === 'commercial_inquiry') return copyFor(language, 'commercialAsk');
  if (intent === 'support_request') return copyFor(language, 'supportAsk');
  return copyFor(language, 'generalAsk');
}
...
// Language policy: reply in the language the customer is actually writing
// in (see detectLanguage in bot-protocol-channel-format.js -- their own
// current message decides this, not IP geolocation), never a fixed
// language. SKU/OEM/part codes and ELIMFILTERS technology names must never
// be translated or altered -- only the surrounding descriptive text is.
async function synthesizeGeneralAnswer(message, state, approvedKnowledge, deterministicAnswer, language = 'en') {
  if (!process.env.GROQ_API_KEY) return deterministicAnswer;
  const targetName = LANGUAGE_NAMES[language] || LANGUAGE_NAMES.en;
  const result = await groqChatJson([
    {
      role: 'system',
      content: `You are ELIMFILTERS' central technical brain. Respond in clear, professional ${targetName} -- the language the customer is writing in -- using the provided history and knowledge. Absolute rules: 1) never invent a SKU, cross-reference, or specification; 2) if there is no catalog evidence, say so explicitly; 3) do not repeat questions already answered; 4) prioritize safety over mechanical risk; 5) never translate or alter SKU codes, part numbers, OEM codes, or ELIMFILTERS product-family/technology names (e.g. SYNTRAX, HYDROCORE, MACROCORE) -- copy them exactly as given, verbatim; every other word of the answer must be in ${targetName}. Return JSON {"answer":"final text"}.`
    },
    {
      role: 'user',
...
async function runBotProtocol(body) {
  ...
  const language = detectLanguage(body, { state: loaded });
  ...
  if (greeting) {
    const payload = buildPayload({
      ...
      answer: copyFor(language, 'greeting'),
  ...
  if (state.phase === 'AWAITING_SUPPORT_EMAIL') {
    ...
    if (email && state.supportLead.created) {
      answer = copyFor(language, 'supportEmailAlreadyRegistered');
    } else if (email) {
      ...
      answer = copyFor(language, 'supportEmailThanks');
    } else {
      answer = copyFor(language, 'supportEmailInvalid');
    }
  ...
  if (action.pendingField) {
    ...
    if (state.installedFilter.reference && verifiedNoMatch(catalog) && !state.supportLead.created) {
      ...
      answer = `${answer}\n\n${copyFor(language, 'noVerifiedMatchReference')} ${copyFor(language, 'supportEmailRequest')}`;
    }
    ...
    if (verifiedNoMatch(catalog) && !state.supportLead.created) {
      state.phase = 'AWAITING_SUPPORT_EMAIL';
      answer = `${copyFor(language, 'noVerifiedMatch')} ${copyFor(language, 'supportEmailRequest')}`;
    } else if (!catalog.products.length) {
      answer = copyFor(language, 'noEvidence');
    } else {
      const lines = catalog.products.map(p => `• ${p.sku}${p.codigo_base ? ` / ${p.codigo_base}` : ''}${p.filter_type ? ` — ${p.filter_type}` : ''}`);
      answer = `${copyFor(language, 'confirmed')}\n\n${lines.join('\n')}`;
    }
  } else {
    answer = directAnswer(state.intent, state, language);
  ...
  if (['specification_lookup', 'general', 'support_request'].includes(state.intent) && pipeline.technicalKnowledge) {
    ...
    answer = await synthesizeGeneralAnswer(message, state, approvedKnowledge, answer, language);
  }
...
async function finalize({ key, state, body, payload, memorySource, contextSeedApplied = false }) {
  ...
  finalPayload = await formatForChannel(finalPayload, body);
