...
Definicion de cada intent (usa la que mejor describe lo que el usuario realmente quiere, no una coincidencia superficial de palabras):
- "application_lookup": el usuario nombra su vehiculo o equipo (marca, modelo, motor y/o anio -- no hace falta que de los cuatro) y pregunta que filtro(s) usa, necesita o lleva. Ejemplos: "que filtro de aceite usa mi Toyota Corolla 2021", "tengo un Freightliner Columbia con Detroit Series 60, que filtros necesita", "mi Ford F-150 2020 que filtro de aire lleva". La marca+modelo del vehiculo (ej. "F-150", "Corolla") NUNCA es un codigo de referencia aunque tenga letras y numeros -- es una entidad de equipo (entities.model), no un installedFilterReference.
- "exact_reference_lookup": el usuario da un codigo de parte/SKU concreto (propio o de ELIMFILTERS) y quiere confirmar o encontrar ese codigo exacto. Ejemplos: "tienen el P551424?", "busco el EF91424".
- "cross_reference_lookup": el usuario da un codigo OEM o de un competidor (Fleetguard, Donaldson, Baldwin, WIX, Mann, Bosch, etc.) y quiere el equivalente ELIMFILTERS. Ejemplos: "equivalente a Fleetguard FS19765", "que cruza con Baldwin B7300".
- "specification_lookup": el usuario pregunta por una medida, dimension, rosca, micronaje, beta ratio o caudal de un producto -- NO por si un vehiculo usa cierto filtro. Ejemplos: "que rosca tiene el EF91424", "cual es el micronaje de este elemento".
- "diagnostic": el usuario describe un sintoma o falla (contaminacion, sobrecalentamiento, perdida de potencia, agua en el sistema, etc.), no pide un producto directamente.
Si el mensaje combina un vehiculo/equipo con una pregunta de "que filtro usa/necesita/lleva", es SIEMPRE "application_lookup", incluso si el modelo del vehiculo parece alfanumerico (F-150, CX-5, RAV4, etc.).
...
async function runCatalogQuery(state, classification, message) {
  ...
  if (intent === 'application_lookup') {
    ...
    // The customer usually names the specific system they need ("filtro de
    // aceite", "filtro de aire"). Without this, an application match returns
    // every filter type on file for that vehicle (oil, air, cabin, fuel...)
    // mixed into one list -- which reads as an unconvincing non-answer
    // instead of the one clear recommendation the customer actually asked
    // for. When no system is named, fall back to the unfiltered set so a
    // generic "what filters does this truck need" question still works.
