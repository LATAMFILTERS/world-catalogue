...
async function queryKnowledgeEngine(message, requestBody, state, candidateCaseId = null) {
  ...
  // The runtime's contract requires correlationId/candidateCaseId to be
  // real UUIDs when present (z.string().uuid().optional()) -- optional
  // accepts a missing key, not an explicit null, and chat session ids
  // (e.g. "diag-final-2") are never UUIDs. Sending either as received
  // failed every single call with HTTP 400 INVALID_REQUEST (confirmed
  // live), so technical-knowledge lookups always silently returned
  // nothing regardless of what was in knowledge_records. Generate a
  // fresh correlation id for tracing instead of reusing the session id,
  // and omit candidateCaseId entirely rather than send it as null.
  ...
  try {
    const response = await fetch(`${url}/api/knowledge-engine/v1/reason`, {
      ...
      body: JSON.stringify({
        ...
        correlationId,
        ...(candidateCaseId && UUID_PATTERN.test(candidateCaseId) ? { candidateCaseId } : {}),
        context: {
          state,
          sessionId,
