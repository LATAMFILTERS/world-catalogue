...
function pageSupportsOfficialProduct(html, donaldsonCode) {
  const text = stripHtml(html);
  const normalizedText = normalizeCode(text);
  const code = normalizeCode(donaldsonCode);
  const hasDonaldsonContext = /donaldson/i.test(text);
  return Boolean(code && hasDonaldsonContext && normalizedText.includes(code));
}
