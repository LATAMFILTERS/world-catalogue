...
const APPLICATION_CANDIDATE_LIMIT = 48;
...
async function searchByApplication(tokens, year = null, filterSystem = null) {
  ...
  try {
    const products = await withProtocolClient(async client => {
      // Push every known application discriminator into PostgreSQL instead of
      // fetching a broad 120-row set from one token and filtering in Node.
      // The pg_trgm expression index can service these ILIKE predicates and
      // dramatically reduces heap/JSON work for brand+model+engine+year lookups.
      const patterns = [...terms, ...(year ? [String(year)] : [])].map(term => `%${term}%`);
      const candidateResult = await client.query(
        `SELECT ${SELECT_FIELDS}, 'application'::text AS protocol_match_type
           FROM elimfilters_catalog c
          WHERE coalesce(c.equipment_applications::text, '') ILIKE ALL($1::text[])
