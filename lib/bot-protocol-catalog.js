...
function collectReferenceCodes(value, output = [], keyHint = null) {
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectReferenceCodes(item, output, keyHint);
    return output;
  }
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (CODE_FIELD_NAMES.has(normalizedKey)) {
      collectReferenceCodes(item, output, normalizedKey);
      continue;
    }

    // brand_crossrefs is keyed by manufacturer and each value is commonly
    // an array of codes. Recurse into arrays/objects, but do not treat scalar
    // metadata such as manufacturer names as part numbers.
...
async function crossReferenceCandidatesFor(client, reference) {
  const result = await client.query(
    `SELECT ${SELECT_FIELDS}, 'cross_reference'::text AS protocol_match_type
      ...
      WHERE regexp_replace(upper(coalesce(c.oem_codes::text, '')), '[^A-Z0-9]', '', 'g') LIKE $1
         OR regexp_replace(upper(coalesce(c.competitor_codes::text, '')), '[^A-Z0-9]', '', 'g') LIKE $1
         OR regexp_replace(upper(coalesce(c.brand_crossrefs::text, '')), '[^A-Z0-9]', '', 'g') LIKE $1
  ...
  try {
    const products = await withProtocolClient(async client => {
      ...
      const candidateGroups = await Promise.all(
        normalized.map(reference => crossReferenceCandidatesFor(client, reference))
      );
