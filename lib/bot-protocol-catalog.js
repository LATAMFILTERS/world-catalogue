...
async function crossReferenceCandidatesFor(client, reference) {
  ...
  const result = await client.query(
    `SELECT ${SELECT_FIELDS}, 'cross_reference'::text AS protocol_match_type
      ...
      WHERE regexp_replace(upper(coalesce(c.oem_codes::text, '')), '[^A-Z0-9]', '', 'g') LIKE ('%' || $1::text || '%')
         OR regexp_replace(upper(coalesce(c.competitor_codes::text, '')), '[^A-Z0-9]', '', 'g') LIKE ('%' || $1::text || '%')
         OR regexp_replace(upper(coalesce(c.brand_crossrefs::text, '')), '[^A-Z0-9]', '', 'g') LIKE ('%' || $1::text || '%')
