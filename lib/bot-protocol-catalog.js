...
// A vehicle/equipment application typically has one entry per filter system
// (oil, air, cabin, fuel, coolant...). Without this, "what filter does my
// Corolla use" and "what oil filter does my Corolla use" return the exact
// same unfiltered mix of every system on file for that vehicle -- which
// buries the one filter the customer actually asked about among unrelated
// ones instead of giving a single clear recommendation.
...
function filterTypeMatchesSystem(filterType, system) {
  ...
  // "Cabin Air Filter" also contains "air" but answers a different question
  // ("aire acondicionado / polen") than intake air filtration.
...
async function searchByApplication(tokens, year = null, filterSystem = null) {
  ...
  try {
    const products = await withProtocolClient(async client => {
      ...
      // A requested system that matches nothing (e.g. mislabeled catalog
      // data) should still show the vehicle's other filters rather than a
      // false "not found" -- silently dropping to the unfiltered set here
      // is the only case where that's acceptable, since it only fires when
      // filtering by system produced zero results in the first place.
