...
async function releaseRenderLeaseIfDisabled() {
  try {
    const { failWeeklyRun } = await import('../scripts/hermes/hermes-ha-control.mjs');
    const released = await failWeeklyRun({ nodeId: 'RENDER', error: 'Failover disabled during recovery; Render lease released.' });
    if (released) {
      console.warn('[hermes-failover-watch] released incomplete Render HA lease because failover is disabled');
    }
  } catch (error) {
    console.error('[hermes-failover-watch] failed to release disabled Render lease', error.message);
  }
}
...
function startRenderFailoverWatcher() {
  if (process.env.ELIM_HERMES_FAILOVER_ENABLED !== 'true') {
    ...
    setTimeout(() => releaseRenderLeaseIfDisabled(), 5000).unref();
    return;
  }
  if (timer) return;
