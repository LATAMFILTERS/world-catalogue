...
function installHermesReviewCenter(app) {
  ...
  app.get(path, (req, res) => {
    ...
    const reasonPrompt = payload.decision === 'research'
      ? 'Indica exactamente qué debe investigar HERMES antes de volver a presentar este hallazgo.'
      : payload.decision === 'reject'
        ? 'Indica por qué rechazas este hallazgo. El motivo quedará en el registro de auditoría.'
        : 'Comentario opcional para el registro de auditoría.';
    const required = requiresReason(payload.decision) ? ' required' : '';
    const placeholder = payload.decision === 'research' ? 'Instrucción de investigación' : payload.decision === 'reject' ? 'Motivo del rechazo' : 'Comentario';
  ...
  app.post(path, async (req, res) => {
    ...
    if (requiresReason(payload.decision) && !reason) {
      return res.status(400).send(page('Falta información', `<div class="card"><h1>No se envió la decisión</h1><p class="error">${payload.decision === 'research' ? 'Debes indicar qué debe investigar HERMES.' : 'Debes indicar el motivo del rechazo.'}</p><p class="muted">Vuelve al correo y abre nuevamente esta decisión.</p></div>`));
    }
    ...
    try {
      await dispatchDecision({ token: githubToken, repository, ref, workflow, payload, reason });
      const label = LABELS[payload.decision] || payload.decision;
