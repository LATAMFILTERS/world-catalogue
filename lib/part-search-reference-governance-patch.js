...
function applyGovernanceToSearchBody(body, rawReference) {
  ...
  if (policy && (body.source === 'governance_quarantine' || body.mixed_duty === true || !Array.isArray(body.results) || body.results.length === 0)) {
    const candidateList = Array.isArray(body.ld_products) && body.ld_products.length ? body.ld_products :
                          Array.isArray(body.hd_products) && body.hd_products.length ? body.hd_products :
                          Array.isArray(body.results) && body.results.length ? body.results : [];
    const filtered = filterProductsForGovernance(candidateList, policy)
      .map(product => ({ ...product, duty: product.duty || policy.duty }));

    const products = filtered.length ? filtered : policy.approvedSkus.map(sku => ({
      sku,
      elimfilters_sku: sku,
      duty: policy.duty,
      filter_type: policy.duty === 'LIGHT_DUTY' ? 'oil' : 'fuel',
      name: `Elimfilters ${sku}`,
    ...
    next.reference_review_required = false;
    return next;
  }
