...
const REFERENCE_GOVERNANCE = Object.freeze({
  'G3802': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EF33802']),
    reason: 'FRAM G3802 is American/Asian Light Duty Gasoline Fuel Filter equivalent to EF33802'
  }),
  'G3802A': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EF33802']),
    reason: 'FRAM G3802A is American/Asian Light Duty Gasoline Fuel Filter equivalent to EF33802'
  }),
  'G3802DP': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EF33802']),
    reason: 'FRAM G3802DP is American/Asian Light Duty Gasoline Fuel Filter equivalent to EF33802'
  }),
  'PH3614': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EL33614']),
    reason: 'FRAM PH3614 is American Light Duty Oil Filter (3/4-16 thread) equivalent to EL33614'
  }),
  'PH3614A': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EL33614']),
    reason: 'FRAM PH3614A is American Light Duty Oil Filter (3/4-16 thread) equivalent to EL33614'
  }),
  'PH3614AZ': Object.freeze({
    duty: 'LIGHT_DUTY',
    approvedSkus: Object.freeze(['EL33614']),
    reason: 'FRAM PH3614AZ is American Light Duty Oil Filter (3/4-16 thread) equivalent to EL33614'
  }),
...
function productSku(product) {
  return product && (product.sku || product.elimfilters_sku);
}
...
function governedDutyForReference(rawReference, requestedDuty) {
  const policy = governanceForReferences([rawReference]);
  return policy?.duty || requestedDuty || null;
}
...
function filterProductsForGovernance(products, policy) {
  ...
  return (Array.isArray(products) ? products : []).filter(product =>
    allowed.has(normalizeReference(productSku(product)))
  );
}
...
function isDirectReferenceProduct(product, references) {
  ...
  return normalized.has(normalizeReference(productSku(product)))
...
function applyGovernanceToSearchBody(body, rawReference) {
  ...
  // Mixed-duty resolution happens upstream before the public-response
  // governance adapter. For an explicitly governed reference, that prompt
  // must not outrank the reviewed duty/SKU allowlist. Resolve from the exact
  // duty bucket and fail closed if none of the approved SKUs are present.
  if (policy && body.mixed_duty === true) {
    const dutyProducts = policy.duty === 'HEAVY_DUTY'
      ? body.hd_products
      : policy.duty === 'LIGHT_DUTY'
        ? body.ld_products
        : [];
    const products = filterProductsForGovernance(dutyProducts, policy)
      .map(product => ({ ...product, duty: product.duty || policy.duty }));
    const totalMixedProducts = arrayLength(body.hd_products) + arrayLength(body.ld_products);

    next.results = products;
    next.mixed_duty = false;
    next.source = products.length ? 'xref_governed' : body.source;
    next.resolution = products.length ? 'RESOLVED' : body.resolution;
    ...
    next.reference_safety_status = 'REFERENCE_GOVERNANCE_DUTY_OVERRIDE';
    next.reference_safety_removed = Math.max(0, totalMixedProducts - products.length);
    if (!products.length) next.reference_review_required = true;
    return next;
  }

  // A caller can send an explicit duty that conflicts with governance. The
  // request middleware normally replaces it before SQL runs; this response
  // filter is the fail-closed second line of defence so stale LD candidates
  // can never leak for an HD-governed reference.
  let governedCandidateRemovals = 0;
  if (policy && Array.isArray(body.candidates)) {
    const allowed = new Set(policy.approvedSkus.map(normalizeReference));
    next.candidates = body.candidates.filter(candidate =>
      allowed.has(normalizeReference(candidate))
    );
    governedCandidateRemovals = body.candidates.length - next.candidates.length;
    if (governedCandidateRemovals > 0) {
      next.source = 'xref_governed';
      next.resolution = next.candidates.length
        ? body.resolution
        : 'GOVERNANCE_BLOCKED_CONFLICTING_DUTY';
    }
  }
  ...
  next.reference_safety_removed = safety.removed + governedCandidateRemovals;
