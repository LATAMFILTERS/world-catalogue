...
// Per the canonical registry (frontend/src/lib/canonical-technologies.ts),
// TURBOCORE™ is scoped specifically to "Turbine FH and FG fuel-separation
// systems" -- not fuel/water separation in general. A generic spin-on
// fuel/water separator that isn't a Racor-style turbine housing belongs to
// SYNTAPORE™ ("Primary and secondary fuel filtration") instead. Assigning
// TURBOCORE™ to every fuel/water separator exposed a technology name outside
// the product family it actually describes.
function isTurbineHousingSignal(product = {}) {
  const haystack = `${product.filter_type || ''} ${product.sub_type || ''} ${product.name || ''} ${product.sku || ''} ${product.codigo_base || ''}`.toUpperCase();
  const codigoBase = String(product.codigo_base || '').toUpperCase();
  if (/TURBINE|RACOR/.test(haystack)) return true;
  // Racor-style turbine housings (900/902/1000/1002 FH/FG).
  if (/(?:^|[^A-Z0-9])(?:500|900|1000|2010|2020|2040)(?:FG|FH)(?:[^A-Z0-9]|$)/.test(haystack)) return true;
  // The replacement elements for those turbines (series 2010/2020/2040, in
  // PM/SM/TM variants) belong to the same TURBOCORE family even though their
  // code doesn't carry an FG/FH suffix -- matches the anchored pattern used
  // in the one-time production data correction for this exact case.
...
function inferTechnology(product = {}) {
  ...
  // `type` covers filter_type/sub_type only -- description/name/category are
  // deliberately excluded here (unlike an earlier independent attempt at this
  // same fix on main). Those fields are free-text and, per production
  // verification, don't reliably carry the turbine signal anyway: the real
  // signal lives in sku/codigo_base (e.g. "900FH", "2010SM-OR"), which is
  // exactly what isTurbineHousingSignal checks.
  const type = `${product.filter_type || ''} ${product.sub_type || ''}`.toLowerCase();
  const isFuelWaterSeparator = /fuel.*water|water.*separator|separador.*agua/.test(type);
  const isTurbine = isTurbineHousingSignal(product);
  if (isFuelWaterSeparator || /fuel|diesel|combustible/.test(type)) return isTurbine ? 'TURBOCORE™' : 'SYNTAPORE™';
