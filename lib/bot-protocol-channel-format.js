...
function inferTechnology(product = {}) {
  ...
  if (/fuel.*water|water.*separator|separador.*agua|turbine|turbina|\b(?:fh|fg)\b/.test(descriptor)) return 'TURBOCORE™';
  if (/fuel|diesel|combustible/.test(descriptor)) return 'SYNTAPORE™';
  if (/hydraulic|hidr[aá]ul/.test(descriptor)) return 'NANOFORCE™';
  if (/lube|lubric|oil|aceite/.test(descriptor)) return 'SYNTRAX™';
  if (/coolant|refrigerante|cooling/.test(descriptor)) return 'THERMACORE™';
  if (/cabin|cabina/.test(descriptor)) return 'MICROKAPPA™';
  if (/air\s*dryer|dryer|secador/.test(descriptor)) return 'DRYCORE™';
  if (/housing|air\s*intake\s*housing|filter\s*housing|carcasa/.test(descriptor)) return 'INTEKCORE™';
  if (/air|aire|intake|admisi[oó]n/.test(descriptor)) return 'MACROCORE™';
  return null;
}
...
function buildCrossReferenceNarrative(payload, requestBody, language) {
  ...
  if (!sku) return null;
  const filterType = cleanText(product.filter_type || product.name || 'filter', 120);
  ...
  if (language === 'en') {
    ...
    const purpose = description ? ` ${description.replace(/[.]+$/, '')}.` : '';
    const tech = technology ? ` It incorporates ELIMFILTERS ${technology} technology to support system cleanliness and asset protection.` : '';
    return `${base}${purpose}${tech}`;
  }
  if (language === 'pt') {
    const base = `O código OEM ${inputReference} corresponde ao ELIMFILTERS ${sku}${filterType ? ` ${filterType}` : ''}.`;
    const purpose = description ? ` ${description.replace(/[.]+$/, '')}.` : '';
    const tech = technology ? ` Incorpora a tecnologia ELIMFILTERS ${technology} para apoiar a limpeza do sistema e a proteção do ativo.` : '';
    return `${base}${purpose}${tech}`;
  }
  const base = `La referencia OEM ${inputReference} cruza con el ELIMFILTERS ${sku}${filterType ? ` ${filterType}` : ''}.`;
  const purpose = description ? ` ${description.replace(/[.]+$/, '')}.` : '';
  const tech = technology ? ` Incorpora tecnología ELIMFILTERS ${technology} para respaldar la limpieza del sistema y la protección del activo.` : '';
  return `${base}${purpose}${tech}`;
}
