...
function inferTechnology(product = {}) {
  ...
  if (/fuel.*water|water.*separator|separador.*agua/.test(type)) return 'HYDROCORE™';
  if (/fuel/.test(type)) return 'HYDROCORE™';
...
function buildCrossReferenceNarrative(payload, requestBody, language) {
  ...
  if (language === 'en') {
    ...
    const tech = technology ? ` It incorporates ELIMFILTERS ${technology} technology to support fuel cleanliness and asset protection.` : '';
