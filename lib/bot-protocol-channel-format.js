...
// is retired and no longer a distinct technology; its former scope
// (Racor-style FH/FG turbine housings) is now governed under HYDROCORE™,
// still as the distinct FUEL_TURBINE product family. A generic spin-on
// fuel/water separator that isn't a turbine housing belongs to the same
// HYDROCORE™ technology, in the FUEL_WATER_SEPARATOR product family instead.
// SYNTAPORE™ ("Primary and secondary fuel filtration") remains scoped to
// plain particulate filtration only, never separators.
function isTurbineHousingSignal(product = {}) {
  ...
  // PM/SM/TM variants) belong to the same turbine housing family even though
  // their code doesn't carry an FG/FH suffix -- matches the anchored pattern
  // used in the one-time production data correction for this exact case.
...
function inferTechnology(product = {}) {
  ...
  // A turbine signal (Racor-style FH/FG housing, the 2010/2020/2040 element
  // series, or an explicit "turbine"/"Racor" mention -- see
  // isTurbineHousingSignal) still identifies the FUEL_TURBINE product family,
  // but the governing technology is HYDROCORE™ for both turbine and
  // non-turbine separators since the TURBOCORE -> HYDROCORE consolidation.
  const isTurbine = isTurbineHousingSignal(product);
  if (isTurbine) return 'HYDROCORE™';
  ...
  // reserved for standard (non-turbine) spin-on/cartridge separators.
