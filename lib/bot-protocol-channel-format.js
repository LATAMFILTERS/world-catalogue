...
function inferTechnology(product = {}) {
  ...
  if (isFuelWaterSeparator || /fuel/.test(type)) return isTurbine ? 'TURBOCORE™' : 'SYNTAPORE™';
  if (/dryer|secador/.test(type)) return 'DRYCORE™';
  if (/hydraulic/.test(type)) return 'NANOFORCE™';
  if (/lube|oil/.test(type)) return 'SYNTRAX™';
  if (/coolant/.test(type)) return 'THERMACORE™';
  if (/cabin/.test(type)) return 'MICROKAPPA™';
  if (/air/.test(type) && /housing/.test(type)) return 'INTEKCORE™';
  if (/air/.test(type)) return 'MACROCORE™';
  return null;
}
