...
// back to Spanish (the table's baseline) if the language or key is missing.
// Use this instead of hardcoding Spanish literals for any customer-facing
// bot response -- see COPY above for the available keys.
function copyFor(language, key) {
  return (COPY[language] && COPY[language][key]) ?? COPY.es[key];
}
