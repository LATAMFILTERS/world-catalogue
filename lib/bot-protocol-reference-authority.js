...
async function fetchValidatedCrossrefs(client, sku) {
  const normalizedSku = normalizeReference(sku);
  if (!normalizedSku) return [];
  const result = await client.query(
    `SELECT code, sku, manufacturer, score, status
       FROM ${RESOLVER_VIEW}
      WHERE upper(regexp_replace(coalesce(sku, ''), '[^A-Z0-9]', '', 'g')) = $1
        AND status = ANY($2::text[])
      ORDER BY score DESC NULLS LAST, manufacturer ASC, code ASC
      LIMIT 100`,
    [normalizedSku, [...RESOLVED_STATUSES]]
  );
  return result.rows || [];
}
...
async function fetchInternalSkuMatches(client, references) {
  ...
  const products = [];
  for (const row of result.rows.map(normalizeProduct)) {
    products.push({
      ...row,
      protocol_source_brand: 'ELIMFILTERS',
      protocol_resolved_reference: row.sku,
      protocol_resolver_status: 'RESOLVED_INTERNAL_SKU',
      protocol_resolver_score: 1000,
      protocol_validated_crossrefs: await fetchValidatedCrossrefs(client, row.sku)
    });
  }
  return products;
}
...
async function resolveReferenceAuthority(references) {
  ...
  try {
    return await withProtocolClient(async client => {
      ...
      const validatedCrossrefs = await fetchValidatedCrossrefs(client, winner.sku);
      const augmented = {
        ...product,
        ...
        protocol_validated_crossrefs: validatedCrossrefs
      };
