...
const MIN_STATEMENT_TIMEOUT_MS = 30000;
...
function resolveStatementTimeout(requestedMs) {
  ...
  const requested = Number.isFinite(numeric) ? numeric : MIN_STATEMENT_TIMEOUT_MS;
  return Math.min(MAX_STATEMENT_TIMEOUT_MS, Math.max(MIN_STATEMENT_TIMEOUT_MS, requested));
}

async function withProtocolClient(callback, { statementTimeoutMs = MIN_STATEMENT_TIMEOUT_MS } = {}) {
