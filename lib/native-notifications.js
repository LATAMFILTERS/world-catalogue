...
const TYPES = new Set([
  ...
  'JOB_STARTED','JOB_PROGRESS','JOB_COMPLETED','JOB_BLOCKED','JOB_FAILED'
]);
...
function crmPushLink(value) {
  const link = clean(value, 1000);
  if (!link) return null;
  if (/^https:\/\//i.test(link) || /^\/push(?:\/|\?|$)/i.test(link)) return link;
  return null;
}

async function projectToCeoPush(notification) {
  if (String(notification?.recipient_key || '').toUpperCase() !== 'CEO') return null;
  const endpoint = clean(process.env.CRM_PUSH_EVENTS_URL, 1000);
  const token = clean(process.env.CRM_INGEST_TOKEN, 4000);
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      headers: {
        ...
        'idempotency-key': clean(notification.dedupe_key, 255) || String(notification.id),
      },
      body: JSON.stringify({
        sourceSystem: 'WORLD_CATALOGUE',
        ...
        deepLink: crmPushLink(notification.deep_link),
        metadata: notification.metadata || {},
        dedupeKey: notification.dedupe_key || null,
      }),
      signal: AbortSignal.timeout(Number(process.env.CRM_PUSH_TIMEOUT_MS) || 2500),
    });
    if (!response.ok) throw new Error(`CRM_PUSH_HTTP_${response.status}`);
    return true;
  } catch (error) {
    console.error('[world-notifications:crm-push]', error.code || error.name || error.message || 'PUSH_ERROR');
    return null;
  }
}
...
module.exports = { createNotification, safeNotify, listNotifications, updateNotificationStatus, summary, projectToCeoPush };
