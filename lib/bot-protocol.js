...
function extractReferences(text) {
  ...
  const years = new Set((source.match(/\b(?:19[8-9]\d|20[0-3]\d)\b/g) || []).map(normalizeReference));
  const matches = source.match(/\b[A-Z]{1,6}[- ]?\d{3,}[A-Z0-9-]*\b/gi) || [];
  return [...new Set(matches.map(normalizeReference).filter(value => value && !years.has(value) && !/^O?20\d{2}$/.test(value)))];
}
...
function classifyIntent(text, references, equipmentTokens, context = {}) {
  ...
  if (/falla|fallando|problema|s[ií]ntoma|ca[ií]da\s+de\s+presi[oó]n|baja\s+presi[oó]n|pierde presi[oó]n|pierde potencia|humo|contaminaci[oó]n|agua en combustible|se apaga|no arranca|restricci[oó]n/i.test(value)) return 'diagnostic';
...
function shortFollowUpValue(message, pendingField) {
  const value = String(message || '').trim();
  if (!value || !pendingField) return null;
  if (pendingField === 'duration' && /^(?:\d+\s*)?(?:minutos?|horas?|d[ií]as?|semanas?|meses?|a[nñ]os?)(?:\s+.*)?$/i.test(value)) return value;
  if (pendingField === 'operating_context' && /carretera|flota|transporte|min(?:a|er[ií]a)|construcci[oó]n|obra|agricultura|agr[ií]cola|generaci[oó]n|marino|industrial/i.test(value)) return value;
  if (pendingField === 'impact' && /detenid[oa]|parad[oa]|fuera\s+de\s+servicio|p[eé]rdida\s+de\s+potencia|consumo|riesgo|producci[oó]n|retraso|downtime|da[nñ]o/i.test(value)) return value;
  return null;
}
...
function extractDiagnosticData(message, context = {}) {
  ...
  const patterns = [
    ...
    ['pressure_loss', /ca[ií]da\s+de\s+presi[oó]n|baja presi[oó]n|pierde presi[oó]n/i], ['restriction', /restricci[oó]n|filtro tapado|obstrucci[oó]n/i]
  ];
  ...
  const pendingValue = shortFollowUpValue(text, context.pending_field);
  const messageEquipment = extractEquipmentTokens(text);
  const storedEquipment = Array.isArray(context.equipment_tokens) ? context.equipment_tokens : [];
  return {
    equipment_tokens: messageEquipment.length ? [...new Set([...storedEquipment, ...messageEquipment])] : storedEquipment.length ? storedEquipment : extractEquipmentTokens(combined),
    symptoms: context.symptoms?.length ? context.symptoms : [...new Set(symptoms)],
    duration: context.duration || (context.pending_field === 'duration' ? pendingValue : null) || durationMatch?.[1]?.trim() || null,
    operating_context: context.operating_context || (context.pending_field === 'operating_context' ? pendingValue : null) || (/mina|minero|miner[ií]a/i.test(combined) ? 'mining' : /construcci[oó]n|obra/i.test(combined) ? 'construction' : /agricultura|agr[ií]cola|cosecha/i.test(combined) ? 'agriculture' : /flota|carretera|transporte/i.test(combined) ? 'fleet' : null),
    impact: context.impact || (context.pending_field === 'impact' ? pendingValue : null) || (/parad[oa]|fuera de servicio|detenido|no puede trabajar/i.test(combined) ? 'equipment_down' : /producci[oó]n|operaci[oó]n|entrega|retraso|downtime/i.test(combined) ? 'operational_loss' : null)
  };
}
...
async function processQuery(message, context = {}) {
  ...
  const equipmentTokens = context.equipment_tokens?.length ? context.equipment_tokens : extractEquipmentTokens(combinedText);
...
function registerBotProtocol(app) {
  app.post('/api/bot/protocol', async (req, res) => {
    ...
    try {
      ...
      console.info('[bot-protocol]', { intent: result.intent, phase: result.phase, pending_field: result.diagnostic?.missing_field || null, duration_ms: Date.now() - startedAt, evidence_count: result.evidence?.count || 0 });
...
module.exports = { registerBotProtocol, processQuery, extractDiagnosticData, nextDiagnosticQuestion, extractYear, strongestApplicationToken, classifyIntent };