...
const { researchApplicationToOems } = require('./bot-protocol-catalog-research');
...
async function resolveApplicationProducts(message, context, diagnostic, tokens) {
  let products = await searchApplications(tokens);
  let research = {
    used: false,
    status: products.length ? 'internal_match' : 'not_run',
    identity: null,
    oem_codes: [],
    sources: []
  };
  let matchType = products.length ? 'internal_application' : null;

  if (!products.length) {
    research = await researchApplicationToOems({ message, context, diagnostic });
    if (research.status === 'confirmed' && research.oem_codes.length) {
      products = await searchReferences(research.oem_codes);
      if (products.length) matchType = 'external_oem_to_internal_sku';
    }
  }

  return { products, research, match_type: matchType };
}
...
async function processQuery(message, context = {}) {
  ...
  const classifiedIntent = classifyIntent(message, references, equipmentTokens);
  const continuingDiagnosticRecommendation = context.active_intent === 'diagnostic' &&
    /qu[eé]\s+filtros?|cu[aá]l(?:es)?\s+filtros?|recomiend|usa|lleva/i.test(String(message || ''));
  const intent = continuingDiagnosticRecommendation ? 'diagnostic' : classifiedIntent;
  ...
  if (intent === 'diagnostic') {
    ...
    let resolution = {
      products: [],
      research: { used: false, status: 'not_run', identity: null, oem_codes: [], sources: [] },
      match_type: null
    };
    if (!next && diagnostic.equipment_tokens.length) {
      resolution = await resolveApplicationProducts(message, context, diagnostic, diagnostic.equipment_tokens);
    }
  ...
  let products = [];
  let research = { used: false, status: 'not_run', identity: null, oem_codes: [], sources: [] };
  let matchType = null;
  if (plan.includes('search_catalog')) {
    products = await searchReferences(references);
    matchType = products.length ? 'exact_reference' : null;
  }
  if (plan.includes('search_applications')) {
    const resolution = await resolveApplicationProducts(message, context, {}, equipmentTokens);
