...
function extractDiagnosticData(message, context = {}) {
  ...
  const patterns = [
    ...
    ['pressure_loss', /baja presi[oó]n|pierde presi[oó]n/i], ['restriction', /restricci[oó]n|filtro tapado|obstrucci[oó]n/i]
  ];
...
async function processQuery(message, context = {}) {
  ...
  const year = extractYear(`${Array.isArray(context.history) ? context.history.join(' ') : ''} ${message}`);
  ...
  if (intent === 'application_lookup' && !year) {
    return {
      protocol_version: '1.2.0', intent, phase: 'collecting_application_data', plan,
      ...
      answer: '¿De qué año es el Freightliner? Con el modelo, motor y año exactos valido la aplicación antes de recomendar un filtro.'
    };
  }
  ...
  return {
    protocol_version: '1.2.0', intent, phase: 'catalog_resolution', plan,
...
function registerBotProtocol(app) {
  app.post('/api/bot/protocol', async (req, res) => {
    ...
    try {
      ...
      console.error('[bot-protocol]', { error: error.message, duration_ms: Date.now() - startedAt });
      return res.status(503).json({ error: 'protocol_temporarily_unavailable' });
    }
  });
}

module.exports = { registerBotProtocol, processQuery, extractDiagnosticData, nextDiagnosticQuestion, extractYear, strongestApplicationToken };
